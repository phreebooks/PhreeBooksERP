...
class order_dl extends base {
  ...
  function check() {
    ...
    if (!isset($this->_check)) {
      $check_query = $admin->DataBase->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PHREEBOOKS_ORDER_DOWNLOAD_STATUS'");
  ...
  function install() {
	...
	$result = $admin->DataBase->Execute("describe " . TABLE_ORDERS);
	$field_exists = false;
	while(!$result->EOF) {
		if ($result->fields['Field'] == 'phreebooks') { $field_exists = true; break; }
		$result->MoveNext();
	}
	if (!$field_exists) $admin->DataBase->Execute("alter table " . TABLE_ORDERS . " add phreebooks tinyint(1) default 0 not null");
	// Add the phreebooks_sku field to the table products
	$result = $admin->DataBase->Execute("describe " . TABLE_PRODUCTS);
	$field_exists = false;
	while(!$result->EOF) {
	  if ($result->fields['Field'] == 'phreebooks_sku') { $field_exists = true; break; }
	  $result->MoveNext();
	}
	if (!$field_exists) $admin->DataBase->Execute("alter table " . TABLE_PRODUCTS . " add phreebooks_sku varchar(24) not null");
  }
