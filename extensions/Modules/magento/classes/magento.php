...
/*************************************************************************************/
  function buildProductUploadXML($id, $inc_image = true) {
	...
	if (MAGENTO_USE_PRICE_SHEETS == '1') {
	  ...
	  if ($default_levels->rowCount() == 0) throw new \core\classes\userException(MAGENTO_ERROR_NO_PRICE_SHEET . MAGENTO_PRICE_SHEET);
...
/*************************************************************************************/
  function buildProductSyncXML() {
	...
	if ($result->rowCount() == 0) throw new \core\classes\userException(MAGENTO_ERROR_NO_ITEMS);
	$this->strXML  = '<?xml version="1.0" encoding="UTF-8" ?>' . chr(10);
	$this->strXML .= '<Request>' . chr(10);
	$this->strXML .= xmlEntry('Version', '2.00');
	$this->strXML .= xmlEntry('UserName', MAGENTO_USERNAME);
...
/*************************************************************************************/
  function buildConfirmXML() {
	...
	if ($result->rowCount() == 0) throw new \core\classes\userException(MAGENTO_ERROR_CONFRIM_NO_DATA);
