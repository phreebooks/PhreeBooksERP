...
$result = $admin->DataBase->query("select display_name, admin_email from " . TABLE_USERS);
while (!$result->EOF){
...
if($cID != ''){
	$result 		= $admin->DataBase->query("select email, primary_name from " . TABLE_ADDRESS_BOOK . " where ref_id = '$cID' and type like '%m'");
	while (!$result->EOF){
