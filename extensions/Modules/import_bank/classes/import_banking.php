	...
	public    $open_inv    = array();
	 ...
	 public function start_import($ouwer_bank_account_number, $post_date, $other_bank_account_number, $credit_amount, $debit_amount, $description, $bank_gl_acct, $other_bank_account_iban){
	 	...
	 	if ($ouwer_bank_account_number <> '') {
			...
			if ($result->fetch(\PDO::FETCH_NUM)== 0) throw new \core\classes\userException(TEXT_THERE_IS_NO_GL_ACCOUNTS_WITH_THE_DESCRIPTION .': '. $ouwer_bank);
			if (!$result->fetch(\PDO::FETCH_NUM)> 1) throw new \core\classes\userException(TEXT_THERE_ARE_TWO_OR_MORE_GL_ACCOUNTS_WITH_THE_DESCRIPTION .': '. $ouwer_bank);
