...
class outputPage  {
    ...
    function send_menu($basis){
    	...
    	echo '  <ul>' . chr(10);
    	foreach((array)$basis->mainmenu as $menu_item) $this->create_menu($menu_item);
    	echo '  </ul>' . chr(10);
...
//    	})
    ...
    function create_menu(array $array){
    	...
    	if(isset($array['submenu'])){
    		...
    		if($this->check_permission($array['submenu'])){
    			echo "  <li><a href='{$array['link']}' {$array['params']}>".(isset($array['icon']) ? $array['icon'].' '.$array['text'] : $array['text']).'</a>';
    			echo '    <ul>';
    			foreach($array['submenu'] as $menu_item) $this->create_menu($menu_item);
    			echo '    </ul>';
    			echo '  </li>';
    		}
    	}else{
    ...
    function check_permission(array $array){
    	...
    	foreach($array as $menu_item){
    		if(is_array($menu_item['submenu'])) {
    			if($this->check_permission($menu_item['submenu'])) $valid = true;
    		}else{
