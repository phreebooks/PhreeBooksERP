...
class admin {
	...
	function check_prerequisites_versions() {
		...
		if (is_array($this->prerequisites) && sizeof($this->prerequisites) > 0) {
			foreach ($this->prerequisites as $module => $RequiredVersion) {
		  		if ( $admin_classes[$module]->installed == false) throw new \Exception (sprintf(ERROR_MODULE_NOT_INSTALLED, $this->id, $admin_classes[$module]->id));
		  		if ( version_compare($admin_classes[$module]->version, $RequiredVersion) < 0 ) throw new \Exception (sprintf(ERROR_MODULE_VERSION_TOO_LOW, $this->id, $admin_classes[$module]->id, $RequiredVersion, $this->version));
			}
		}
		return true;
	}
