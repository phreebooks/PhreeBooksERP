...
if (version_compair($db_release,'1.0', '==')) { // go to Release 1.1
	...
	$admin->DataBase->query("alter table " . TABLE_JOURNAL_MAIN . " change `purch_order_id` `purch_order_id` VARCHAR(24) NULL DEFAULT NULL");
	...
	if (!in_array('next_cm_num', $field_array)) {
	  	$admin->DataBase->query("alter table " . TABLE_CURRENT_STATUS . " add next_cm_num VARCHAR(16) NOT NULL");
	  	$admin->DataBase->query("update " . TABLE_CURRENT_STATUS . " set next_cm_num = 'CM1000'");
	}
	if (!in_array('next_ap_quote_num', $field_array)) {
	  	$admin->DataBase->query("alter table " . TABLE_CURRENT_STATUS . " add next_ap_quote_num VARCHAR(16) NOT NULL");
	  	$admin->DataBase->query("update " . TABLE_CURRENT_STATUS . " set next_ap_quote_num = 'RFQ1000'");
	}
	if (!in_array('next_ar_quote_num', $field_array)) {
	  	$admin->DataBase->query("alter table " . TABLE_CURRENT_STATUS . " add next_ar_quote_num VARCHAR(16) NOT NULL");
	  	$admin->DataBase->query("update " . TABLE_CURRENT_STATUS . " set next_ar_quote_num = 'Q1000'");
	}
...
if (version_compair($db_release,'1.1', '==')) { // go to Release 1.2
	...
	if (!in_array('actual_date', $field_array)) {
	  	$admin->DataBase->query("alter table " . TABLE_SHIPPING_LOG . " ADD actual_date DATE NOT NULL DEFAULT '0000-00-00' after deliver_date");
	}
	...
	if (in_array('recur_remain', $field_array)) {
		$admin->DataBase->query("ALTER TABLE " . TABLE_JOURNAL_MAIN . " DROP recur_times");
		$admin->DataBase->query("ALTER TABLE " . TABLE_JOURNAL_MAIN . " CHANGE recur_remain recur_id INT( 11 ) NULL DEFAULT NULL");
	}
	...
	if (in_array('usage_id', $field_array)) {
		$admin->DataBase->query("ALTER TABLE " . TABLE_INVENTORY_HISTORY . " DROP usage_id");
		$admin->DataBase->query("CREATE TABLE " . TABLE_INVENTORY_COGS_USAGE  ." (
	...
	if (!mysql_num_rows(mysql_query("SHOW TABLES LIKE '" . TABLE_INVENTORY_MS_LIST . "'"))) {
		$admin->DataBase->query("CREATE TABLE " . TABLE_INVENTORY_MS_LIST . " (
		  id int(11) NOT NULL auto_increment,
	...
	if (mysql_num_rows(mysql_query("SHOW TABLES LIKE '" . TABLE_ADDRESS_FORMAT . "'"))) {
		$admin->DataBase->query("DROP TABLE " . TABLE_ADDRESS_FORMAT);
	}

	if (!defined('ADDRESS_BOOK_EMAIL_REQUIRED')) {
		$admin->DataBase->query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, last_modified, date_added, use_function, set_function)
	...
	if (in_array('countries_iso_code_2', $field_array)) {
		$admin->DataBase->query("ALTER TABLE " . TABLE_ZONES . " DROP countries_iso_code_2, DROP zone_country_id");
		$admin->DataBase->query("ALTER TABLE " . TABLE_ZONES . " DROP INDEX countries_iso_code_2");
	}

	$admin->DataBase->query("ALTER TABLE " . TABLE_INVENTORY_ASSY_LIST . " ENGINE = innodb");
...
if (version_compair($db_release,'1.2.1', '==')) { // go to Release 1.3
	...
	$admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION_GROUP . " set configuration_group_id = '20'
	  where configuration_group_title = 'Website Maintenance'");
	$admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION_GROUP . " set configuration_group_id = '19'
	  where configuration_group_title = 'Layout Settings'");
	$admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION_GROUP . " set configuration_group_id = '15'
	  where configuration_group_title = 'Sessions'");
	$admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION_GROUP . " set configuration_group_id = '13'
	  where configuration_group_title = 'General Ledger Defaults'");
	$admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION_GROUP . " set configuration_group_id = '6'
	  where configuration_group_title = 'Module Defaults'");
	$admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION_GROUP . " set configuration_group_id = '17'
	...
	$admin->DataBase->query("ALTER TABLE " . TABLE_REPORTS . " CHANGE `description` `description`
	...
	while (!$result->EOF) {
		...
		$admin->DataBase->query("ALTER TABLE " . $tablename . " DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci");
		$result->MoveNext();
	}
...
if (version_compair($db_release,'1.3', '==')) { // go to Release 1.3.1
	...
	if (!mysql_num_rows(mysql_query("SHOW TABLES LIKE '" . TABLE_ACCOUNTS_NOTES . "'"))) {
	  $admin->DataBase->query("CREATE TABLE " . TABLE_ACCOUNTS_NOTES . " (
	    id int(11) NOT NULL auto_increment,
	    notes text,
	    PRIMARY KEY (id)
	...
	$admin->DataBase->query("ALTER TABLE " . TABLE_CURRENT_STATUS . "
...
if (!in_array('currencies_code', $field_array)) {
  $admin->DataBase->query("ALTER TABLE " . TABLE_JOURNAL_MAIN . " ADD `currencies_code` CHAR(3) NOT NULL DEFAULT '" . DEFAULT_CURRENCY . "' AFTER `total_amount` ,
...
if (!in_array('next_vcm_num', $field_array)) {
  $admin->DataBase->query("ALTER TABLE " . TABLE_CURRENT_STATUS . " ADD next_vcm_num VARCHAR(16) NOT NULL AFTER next_cm_num");
  $admin->DataBase->query("update " . TABLE_CURRENT_STATUS . " set next_vcm_num = 'VCM1000'");
}
...
$admin->DataBase->query("ALTER TABLE " . TABLE_RECONCILIATION . " CHANGE `statement_balance` `statement_balance` DOUBLE NOT NULL DEFAULT '0'");

if (!defined('ENABLE_SHIPPING_FUNCTIONS')) {
  $admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION . " SET configuration_group_id = '11' WHERE configuration_key = 'ADDRESS_BOOK_CONTACT_REQUIRED'");
  $admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION . " SET configuration_group_id = '11' WHERE configuration_key = 'ADDRESS_BOOK_ADDRESS1_REQUIRED'");
  $admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION . " SET configuration_group_id = '11' WHERE configuration_key = 'ADDRESS_BOOK_ADDRESS2_REQUIRED'");
  $admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION . " SET configuration_group_id = '11' WHERE configuration_key = 'ADDRESS_BOOK_CITY_TOWN_REQUIRED'");
  $admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION . " SET configuration_group_id = '11' WHERE configuration_key = 'ADDRESS_BOOK_STATE_PROVINCE_REQUIRED'");
  $admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION . " SET configuration_group_id = '11' WHERE configuration_key = 'ADDRESS_BOOK_POSTAL_CODE_REQUIRED'");
  $admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION . " SET configuration_group_id = '11' WHERE configuration_key = 'ADDRESS_BOOK_EMAIL_REQUIRED'");
  $admin->DataBase->query("UPDATE " . TABLE_CONFIGURATION . " SET configuration_group_id = '11' WHERE configuration_key = 'ADDRESS_BOOK_TELEPHONE1_REQUIRED'");
  $admin->DataBase->query("INSERT INTO " .  TABLE_CONFIGURATION . "
           ( `configuration_title` , `configuration_key` , `configuration_value` , `configuration_description` , `configuration_group_id` , `sort_order` , `last_modified` , `date_added` , `use_function` , `set_function` )
  ...
  $admin->DataBase->query("ALTER TABLE " . TABLE_ADDRESS_BOOK . " ADD `telephone1` VARCHAR( 20 ) NULL DEFAULT '' AFTER `country_code` ,
  ...
  $admin->DataBase->query("ALTER TABLE " . TABLE_ADDRESS_BOOK . " CHANGE `notes` `notes` TEXT CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL DEFAULT ''");
  $admin->DataBase->query("ALTER TABLE " . TABLE_ACCOUNTS_HISTORY . " CHANGE `purchase_invoice_id` `purchase_invoice_id` VARCHAR( 24 ) NULL DEFAULT ''");
  $admin->DataBase->query("ALTER TABLE " . TABLE_ACCOUNTS_HISTORY . " CHANGE `journal_id` `journal_id` INT(2) NOT NULL DEFAULT '0'");
...
if (in_array('telephone1', $field_array)) {
  ...
  $admin->DataBase->query("ALTER TABLE " . TABLE_ACCOUNTS . " DROP `telephone1` ,
  ...
  $admin->DataBase->query("DROP TABLE " . TABLE_ACCOUNTS_NOTES);
  $admin->DataBase->query("INSERT INTO " .  TABLE_CONFIGURATION . "
           ( `configuration_title` , `configuration_key` , `configuration_value` , `configuration_description` , `configuration_group_id` , `sort_order` , `last_modified` , `date_added` , `use_function` , `set_function` )
  ...
  $admin->DataBase->query("CREATE TABLE " . TABLE_DATA_SECURITY . " (
    id int(11) NOT NULL auto_increment,
  ...
  $admin->DataBase->query("ALTER TABLE " . TABLE_JOURNAL_MAIN . " ADD bill_telephone1 VARCHAR(20) NULL AFTER bill_country_code,
    ADD bill_email VARCHAR(48) NULL AFTER bill_telephone1");
  $admin->DataBase->query("ALTER TABLE " . TABLE_JOURNAL_MAIN . " ADD ship_telephone1 VARCHAR(20) NULL AFTER ship_country_code ,
...
if (in_array('papersize', $field_array)) {
	...
	while (!$result->EOF) {
		...
		$admin->DataBase->query("insert into " . TABLE_REPORT_FIELDS . " set
    ...
    $admin->DataBase->query("ALTER TABLE " . TABLE_REPORTS . " DROP `narrative` ,
...
if (mysql_num_rows(mysql_query("SHOW TABLES LIKE '" . TABLE_ACCOUNTS . "'"))) {
  $admin->DataBase->query("RENAME TABLE " . TABLE_ACCOUNTS . " TO " . TABLE_CONTACTS . ";");
}
...
if (false) {
  $admin->DataBase->query("ALTER TABLE " . TABLE_INVENTORY_HISTORY   . " ADD store_id INT(11) NOT NULL DEFAULT '0' AFTER ref_id");
  $admin->DataBase->query("ALTER TABLE " . TABLE_INVENTORY_HISTORY   . " ADD INDEX (store_id)");
  $admin->DataBase->query("ALTER TABLE " . TABLE_INVENTORY_COGS_OWED . " ADD store_id INT(11) NOT NULL DEFAULT '0' AFTER journal_main_id");
  $admin->DataBase->query("ALTER TABLE " . TABLE_INVENTORY_COGS_OWED . " ADD INDEX (store_id)");
}
?>