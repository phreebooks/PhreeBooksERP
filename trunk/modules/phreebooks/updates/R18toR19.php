...
if (!$admin->DataBase->field_exists(TABLE_JOURNAL_MAIN, 'printed'))  {
  ...
  $admin->DataBase->query("ALTER TABLE " . TABLE_JOURNAL_MAIN . " ADD INDEX (closed)");
  $admin->DataBase->query("ALTER TABLE " . TABLE_JOURNAL_MAIN . " ADD INDEX (bill_acct_id)");
}
if (!$admin->DataBase->field_exists(TABLE_JOURNAL_ITEM, 'project_id'))  {
  $admin->DataBase->query("ALTER TABLE " . TABLE_JOURNAL_ITEM . " ADD project_id VARCHAR(16) NULL AFTER serialize_number");
}
if (!$admin->DataBase->table_exists(TABLE_PROJECTS_PHASES))  {
  $admin->DataBase->query("CREATE TABLE " . TABLE_PROJECTS_PHASES . " (
...
if (!$admin->DataBase->table_exists(TABLE_PROJECTS_COSTS))  {
  $admin->DataBase->query("CREATE TABLE " . TABLE_PROJECTS_COSTS . " (
...
$result = $admin->DataBase->query("update " . TABLE_JOURNAL_MAIN . " m inner join " . TABLE_JOURNAL_ITEM . " i on m.id = i.ref_id
...
$result = $admin->DataBase->query("ALTER TABLE " . TABLE_JOURNAL_MAIN . " CHANGE printed printed INT( 11 ) NOT NULL DEFAULT '0'");
