...
if (!db_field_exists(TABLE_JOURNAL_MAIN, 'printed'))  {
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_JOURNAL_MAIN . " ADD printed enum('0','1') NOT NULL DEFAULT '0' AFTER closed");
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_JOURNAL_MAIN . " ADD INDEX (closed)");
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_JOURNAL_MAIN . " ADD INDEX (bill_acct_id)");
}
if (!db_field_exists(TABLE_JOURNAL_ITEM, 'project_id'))  {
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_JOURNAL_ITEM . " ADD project_id VARCHAR(16) NULL AFTER serialize_number");
}
if (!db_table_exists(TABLE_PROJECTS_PHASES))  {
  $admin->DataBase->Execute("CREATE TABLE " . TABLE_PROJECTS_PHASES . " (
...
if (!db_table_exists(TABLE_PROJECTS_COSTS))  {
  $admin->DataBase->Execute("CREATE TABLE " . TABLE_PROJECTS_COSTS . " (
...
$result = $admin->DataBase->Execute("update " . TABLE_JOURNAL_MAIN . " m inner join " . TABLE_JOURNAL_ITEM . " i on m.id = i.ref_id 
...
$result = $admin->DataBase->Execute("ALTER TABLE " . TABLE_JOURNAL_MAIN . " CHANGE printed printed INT( 11 ) NOT NULL DEFAULT '0'");
