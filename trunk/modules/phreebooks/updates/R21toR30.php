...
if (!db_field_exists(TABLE_JOURNAL_MAIN, 'closed_date')) {
  ...
  foreach ($toBeRemoved as $value) {
    $admin->DataBase->Execute("delete from " . TABLE_CONFIGURATION . " where configuration_key = '" . $value . "'");
  }
  // change True's and False's to 1's and 0s
  $admin->DataBase->Execute("update " . TABLE_CONFIGURATION . " set configuration_value = '0'
    where configuration_value in ('false', 'False', 'FALSE', 'no', 'No', 'NO')");
  $admin->DataBase->Execute("update " . TABLE_CONFIGURATION . " set configuration_value = '1'
    where configuration_value in ('true', 'True', 'TRUE', 'yes', 'Yes', 'YES')");
  // increase field length to description
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_JOURNAL_MAIN . " ADD `closed_date` DATE NOT NULL AFTER `closed`");
}
...
$result = $admin->DataBase->Execute("select id, doc_title from " . TABLE_PHREEFORM . " where doc_ext in ('rpt','frm')");
$new_reports = array();
while (!$result->EOF) {
  $new_reports[$result->fields['id']] = $result->fields['doc_title'];
  $result->MoveNext();
}
$result = $admin->DataBase->Execute("select id, params from " . TABLE_USERS_PROFILES . " where dashboard_id = 'favorite_reports'");
while (!$result->EOF) {
  if ($result->fields['params']) {
    ...
    $admin->DataBase->Execute("update " . TABLE_USERS_PROFILES . " set params = '" . $params . "' where id = " . $result->fields['id']);
  }
  $result->MoveNext();
}
...
if (db_field_exists(TABLE_CONFIGURATION, 'set_function')) {
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_CONFIGURATION . " DROP configuration_title, DROP configuration_description,
...
if (!db_field_exists(TABLE_CONTACTS, 'tax_id')) {
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_CONTACTS . " ADD tax_id INT(11) NOT NULL DEFAULT '0' AFTER price_sheet");
}

if (!db_field_exists(TABLE_SHIPPING_LOG, 'deliver_late')) {
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_SHIPPING_LOG . " ADD deliver_late ENUM('0','T','L') NOT NULL DEFAULT '0' AFTER actual_date");
}

if (!db_field_exists(TABLE_INVENTORY, 'quantity_on_allocation')) {
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_INVENTORY . " ADD quantity_on_allocation FLOAT NOT NULL DEFAULT '0' AFTER quantity_on_sales_order");
}
