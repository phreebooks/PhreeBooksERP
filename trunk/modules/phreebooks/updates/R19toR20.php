...
if (!db_field_exists(TABLE_CHART_OF_ACCOUNTS, 'heading_only'))  {
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_CHART_OF_ACCOUNTS . " ADD heading_only ENUM('0', '1') NOT NULL DEFAULT '0' AFTER description");
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_CHART_OF_ACCOUNTS . " ADD INDEX (heading_only)");
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_CHART_OF_ACCOUNTS . " DROP subaccount");
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_CHART_OF_ACCOUNTS . " DROP next_ref");
}
...
$result = $admin->DataBase->Execute("select * from " . TABLE_USERS_PROFILES . " order by user_id, menu_id, column_id, row_id");
$row       = 1;
$last_user = 0;
$last_page = '';
$last_col  = 0;
while(!$result->EOF) {
  ...
  $admin->DataBase->Execute("update " . TABLE_USERS_PROFILES . " set row_id = $row where id = " . $result->fields['id']);
  $row++;
  $result->MoveNext();
...
if (!db_field_exists(TABLE_CURRENCIES, 'decimal_precise'))  {
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_CURRENCIES . " ADD decimal_precise CHAR(1) NOT NULL DEFAULT '2' AFTER decimal_places");
  $admin->DataBase->Execute("UPDATE " . TABLE_CURRENCIES . " set decimal_precise = decimal_places");
}
if (!db_field_exists(TABLE_INVENTORY, 'price_sheet'))  {
  // add price_sheet field to inventory table
  $admin->DataBase->Execute("ALTER TABLE " . TABLE_INVENTORY . " ADD price_sheet VARCHAR(32) NULL AFTER cost_method");
}
...
$result = $admin->DataBase->Execute("select id from " . TABLE_CHART_OF_ACCOUNTS. " where account_type = 0 order by id");
$account_array = array();
while (!$result->EOF) {
  $account_array[] = $result->fields['id'];
  $result->MoveNext();
}
$result = $admin->DataBase->Execute("SELECT id FROM " . TABLE_JOURNAL_MAIN . " WHERE closed = '1' and journal_id in (2, 18, 20)");
while (!$result->EOF) {
  $admin->DataBase->Execute("UPDATE " . TABLE_JOURNAL_ITEM . " SET reconciled = 1 WHERE
