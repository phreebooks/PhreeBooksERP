...
if (!db_field_exists(TABLE_CHART_OF_ACCOUNTS, 'heading_only'))  {
  ...
  $admin->DataBase->query("ALTER TABLE " . TABLE_CHART_OF_ACCOUNTS . " DROP next_ref");
}
...
$result = $admin->DataBase->query("select * from " . TABLE_USERS_PROFILES . " order by user_id, menu_id, column_id, row_id");
$row       = 1;
$last_user = 0;
$last_page = '';
$last_col  = 0;
while(!$result->EOF) {
  ...
  $admin->DataBase->query("update " . TABLE_USERS_PROFILES . " set row_id = $row where id = " . $result->fields['id']);
  $row++;
  $result->MoveNext();
...
$result = $admin->DataBase->query("select id from " . TABLE_CHART_OF_ACCOUNTS. " where account_type = 0 order by id");
$account_array = array();
while (!$result->EOF) {
  $account_array[] = $result->fields['id'];
  $result->MoveNext();
}
$result = $admin->DataBase->query("SELECT id FROM " . TABLE_JOURNAL_MAIN . " WHERE closed = '1' and journal_id in (2, 18, 20)");
while (!$result->EOF) {
  $admin->DataBase->query("UPDATE " . TABLE_JOURNAL_ITEM . " SET reconciled = 1 WHERE
