...
class todays_s_quotes extends \core\classes\ctl_panel {
	public $description	 		= TEXT_TODAYS_SALES_QUOTES_DESCRIPTION;
	public $security_id  		= SECURITY_ID_SALES_QUOTE;
	public $text		 		= TEXT_TODAYS_SALES_QUOTES;
	public $version      		= '4.0';
	
	function panelContent(){
	?>
		<table id='todays_sales_quotes' >
	    	<thead>
	    		<tr>
...
<?php if($_SESSION['user']->is_role == 0) echo "store_id:{$_SESSION['user']->admin_prefs['def_store_id']},"?> 
			...
			onBeforeLoad:function(){
				console.log('loading of the todays sales quotes datagrid');
			},
			onLoadSuccess: function(data){
				console.log('the loading of the todays sales quotes was succesfull');
				$.messager.progress('close');
				if(data.error_message) $.messager.alert('<?php echo TEXT_ERROR?>',data.error_message);
			},
			onLoadError: function(){
				console.error('the loading of the todays sales quotes resulted in a error');
				$.messager.progress('close');
				$.messager.alert('<?php echo TEXT_ERROR?>','Load error for table todays sales quotes');
			},
			onDblClickRow: function(index , row){
				console.log('a row in the todays sales quotes was double clicked');
				//@todo open order
			},
			pagination: true,
