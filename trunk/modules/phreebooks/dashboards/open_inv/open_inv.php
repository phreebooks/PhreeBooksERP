	...
	?>
		...
		$('#open_invoices').datagrid({
			...
			onBeforeLoad:function(){
				console.log('loading of the open invoices datagrid');
			},
			onLoadSuccess: function(data){
				console.log('the loading of the open invoices was succesfull');
				$.messager.progress('close');
			},
			onLoadError: function(){
				console.error('the loading of the open invoices resulted in a error');
				$.messager.progress('close');
				$.messager.alert('<?php echo TEXT_ERROR?>','Load error for table open invoices branch');
			},
			onDblClickRow: function(index , row){
				console.log('a row in the open invoices was double clicked');
				//@todo open order
			},
			pagination: true,
