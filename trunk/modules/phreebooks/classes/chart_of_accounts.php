...
class chart_of_accounts {
  ...
  function btn_save($id = '') {
	...
	if (!$this->primary_acct_id == ''){
	    $result = $admin->DataBase->query("select account_type from " . $this->db_table . " where id = '" . $this->primary_acct_id . "'");
  ...
  function build_main_html() {
    ...
    $result = $admin->DataBase->query("select id, description, heading_only, primary_acct_id, account_type, account_inactive from " . $this->db_table);
    $rowCnt = 0;
	while (!$result->EOF) {
