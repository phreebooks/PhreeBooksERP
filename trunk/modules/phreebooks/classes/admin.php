...
class admin extends \core\classes\admin {
	...
	function install($path_my_files, $demo = false) {
		...
		if (! db_field_exists ( TABLE_CURRENT_STATUS, 'next_po_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " ADD next_po_num VARCHAR( 16 ) NOT NULL DEFAULT '5000';" );
		if (! db_field_exists ( TABLE_CURRENT_STATUS, 'next_so_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " ADD next_so_num VARCHAR( 16 ) NOT NULL DEFAULT '10000';" );
		if (! db_field_exists ( TABLE_CURRENT_STATUS, 'next_inv_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " ADD next_inv_num VARCHAR( 16 ) NOT NULL DEFAULT '20000';" );
		if (! db_field_exists ( TABLE_CURRENT_STATUS, 'next_check_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " ADD next_check_num VARCHAR( 16 ) NOT NULL DEFAULT '100';" );
		if (! db_field_exists ( TABLE_CURRENT_STATUS, 'next_deposit_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " ADD next_deposit_num VARCHAR( 16 ) NOT NULL DEFAULT '';" );
		if (! db_field_exists ( TABLE_CURRENT_STATUS, 'next_cm_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " ADD next_cm_num VARCHAR( 16 ) NOT NULL DEFAULT 'CM1000';" );
		if (! db_field_exists ( TABLE_CURRENT_STATUS, 'next_vcm_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " ADD next_vcm_num VARCHAR( 16 ) NOT NULL DEFAULT 'VCM1000';" );
		if (! db_field_exists ( TABLE_CURRENT_STATUS, 'next_ap_quote_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " ADD next_ap_quote_num VARCHAR( 16 ) NOT NULL DEFAULT 'RFQ1000';" );
		if (! db_field_exists ( TABLE_CURRENT_STATUS, 'next_ar_quote_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " ADD next_ar_quote_num VARCHAR( 16 ) NOT NULL DEFAULT 'QU1000';" );
	...
	function delete($path_my_files) {
		...
		if (db_field_exists ( TABLE_CURRENT_STATUS, 'next_so_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " DROP next_so_num" );
		if (db_field_exists ( TABLE_CURRENT_STATUS, 'next_inv_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " DROP next_inv_num" );
		if (db_field_exists ( TABLE_CURRENT_STATUS, 'next_check_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " DROP next_check_num" );
		if (db_field_exists ( TABLE_CURRENT_STATUS, 'next_deposit_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " DROP next_deposit_num" );
		if (db_field_exists ( TABLE_CURRENT_STATUS, 'next_cm_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " DROP next_cm_num" );
		if (db_field_exists ( TABLE_CURRENT_STATUS, 'next_vcm_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " DROP next_vcm_num" );
		if (db_field_exists ( TABLE_CURRENT_STATUS, 'next_ap_quote_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " DROP next_ap_quote_num" );
		if (db_field_exists ( TABLE_CURRENT_STATUS, 'next_ar_quote_num' ))
			$db->Execute ( "ALTER TABLE " . TABLE_CURRENT_STATUS . " DROP next_ar_quote_num" );
	}
