...
class journal_06 extends \core\classes\journal {
	public $id;
	public $recur_id;
	...
	public $closed 				= false;
	public $gl_type             = 'por';
	public $popup_form_type		= 'vend:inv';
	...
	public $text_column_1_title	= TEXT_PO_BAL;
	public $text_column_2_title	= TEXT_RECEIVED;
	public $text_order_closed	= TEXT_INVOICE_PAID;
	...
	public $description 		= TEXT_PURCHASE_OR_RECEIVE_INVENTORY;
		...
		for ($i = 0; $i < $item_rows_to_process; $i++) {
			if ($this->journal_rows[$i]['sku']) {
				...
				if ($inv_list['so_po_item_ref_id']) { // check for reference to po/so to adjust qty on order/sales order
					...
					$this->update_inventory_status($inv_list['sku'], 'quantity_on_order', $adjustment);
				}
			}
		}
		// build the cogs rows
	...
	function validate_purchase_invoice_id() {
		...
		if ($this->purchase_invoice_id <> '') {	// entered a so/po/invoice value, check for dups
			$sql = "SELECT purchase_invoice_id FROM " . TABLE_JOURNAL_MAIN . " WHERE purchase_invoice_id = '{$this->purchase_invoice_id}' and journal_id = '6'";
			if ($this->id) $sql .= " and id <> " . $this->id;
			$result = $admin->DataBase->query($sql);
			if ($result->fetch(\PDO::FETCH_NUM) > 0) throw new \core\classes\userException(sprintf(TEXT_THE_YOU_ENTERED_IS_A_DUPLICATE,_PLEASE_ENTER_A_NEW_UNIQUE_VALUE_ARGS, $journal_types_list[6]['id_field_name']));
	...
	function post_ordr($action) {
		...
		// ***************************** END TRANSACTION *******************************
		$messageStack->add(sprintf(TEXT_SUCCESSFULLY_ARGS, TEXT_POSTED, $journal_types_list[6]['id_field_name'], $this->purchase_invoice_id), 'success');
		return true;
	}
	...
	function add_total_journal_row() {	// put total value into ledger row array
		$this->journal_rows[] = array( // record for accounts receivable
				...
				'description'             => $journal_types_list[6]['text'] . ' - ' . TEXT_TOTAL,
	...
	function add_discount_journal_row() { // put discount into journal row array
		if ($this->discount <> 0) {
			$this->journal_rows[] = array(
					...
					'description'             => $journal_types_list[6]['text'] . ' - ' . TEXT_DISCOUNT,
	...
	function add_freight_journal_row() {	// put freight into journal row array
		...
		if ($this->freight) { // calculate freight charges
			$this->journal_rows[] = array(
					...
					'description'             => $journal_types_list[6]['text'] . ' - ' . TEXT_SHIPPING,
