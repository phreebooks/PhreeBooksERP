...
// +-----------------------------------------------------------------+
//  Path: /modules/phreebooks/classes/journal/journal_12.php
//
// Sales/Invoice Journal (12)
namespace phreebooks\classes\journal;
class journal_12 extends \core\classes\journal {
	public $id;
	public $recur_id;
	...
	public $closed 				= false;
	public $gl_type             = 'sos';
	public $popup_form_type		= 'cust:inv';
	...
	public $text_column_1_title	= TEXT_SO_BAL;
	public $text_column_2_title	= TEXT_QUANTITY;
	...
	function calculate_COGS($item, $return_cogs = false) {
		...
		if ($sql->fetch(\PDO::FETCH_NUM) == 0) {
			...
			$id = $this->inventory_auto_add($item['sku'], $item['description'], 0, $item['price']);
	...
	function validate_purchase_invoice_id() {
		...
		if ($this->purchase_invoice_id <> '') {	// entered a so/po/invoice value, check for dups
			$sql = "SELECT purchase_invoice_id FROM " . TABLE_JOURNAL_MAIN . " WHERE purchase_invoice_id = '{$this->purchase_invoice_id}' and journal_id = '12'";
			if ($this->id) $sql .= " and id <> " . $this->id;
			$result = $admin->DataBase->query($sql);
			if ($result->fetch(\PDO::FETCH_NUM) > 0) throw new \core\classes\userException(sprintf(TEXT_THE_YOU_ENTERED_IS_A_DUPLICATE,_PLEASE_ENTER_A_NEW_UNIQUE_VALUE_ARGS, $journal_types_list[12]['id_field_name']));
	...
	function increment_purchase_invoice_id($force = false) {
		...
		if ($this->purchase_invoice_id == '' || $force) { // increment the po/so/invoice number
			...
			$result = $admin->DataBase->exec($sql);
			if ($result->AffectedRows() <> 1) throw new \core\classes\userException(sprintf(TEXT_THERE_WAS_AN_ERROR_INCREMENTING_THE_ARGS, $journal_types_list[12]['id_field_name']));
		}
	...
	function post_ordr($action) {
		...
		// ***************************** END TRANSACTION *******************************
		$messageStack->add(sprintf(TEXT_SUCCESSFULLY_ARGS, TEXT_POSTED, $journal_types_list[12]['id_field_name'], $this->purchase_invoice_id), 'success');
		return true;
	}
	...
	function add_total_journal_row() {	// put total value into ledger row array
		$this->journal_rows[] = array( // record for accounts receivable
				...
				'description'  => $journal_types_list[12]['text'] . ' - ' . TEXT_TOTAL,
	...
	function add_discount_journal_row() { // put discount into journal row array
		if ($this->discount <> 0) {
			$this->journal_rows[] = array(
					...
					'description'  => $journal_types_list[12]['text'] . ' - ' . TEXT_DISCOUNT,
	...
	function add_freight_journal_row() {	// put freight into journal row array
		...
		if ($this->freight) { // calculate freight charges
			$this->journal_rows[] = array(
					...
					'description'   => $journal_types_list[12]['text'] . ' - ' . TEXT_SHIPPING,
