...
// +-----------------------------------------------------------------+
//  Path: /modules/phreebooks/classes/journal/journal_3.php
//
...
class journal_03 extends \core\classes\journal {//@todo should extend orders
	...
	function unPost_inventory() {
		...
		// journals that don't affect inventory, return now
	...
	function validate_purchase_invoice_id() {
		...
		if ($this->purchase_invoice_id <> '') {	// entered a so/po/invoice value, check for dups
			...
			if ($result->rowCount() > 0) throw new \core\classes\userException(sprintf(TEXT_THE_YOU_ENTERED_IS_A_DUPLICATE,_PLEASE_ENTER_A_NEW_UNIQUE_VALUE_ARGS, $journal_types_list[3]['id_field_name']));
			...
			$result = $admin->DataBase->query("SELECT next_ap_quote_num FROM " . TABLE_CURRENT_STATUS . " LIMIT 1");
			if ($result->rowCount() == 0) throw new \core\classes\userException(sprintf(GL_ERROR_CANNOT_FIND_NEXT_ID, TABLE_CURRENT_STATUS));
			$this->journal_main_array['purchase_invoice_id'] = $result['next_ap_quote_num'];
	...
	function add_freight_journal_row($debit_credit) {	// put freight into journal row array
		...
		$freight_tax_id = $tax_freight ? AP_ADD_SALES_TAX_TO_SHIPPING : 0; break;
