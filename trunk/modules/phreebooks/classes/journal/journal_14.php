...
class journal_14 extends \core\classes\journal {
	...
	function validate_purchase_invoice_id() {
		...
		if ($this->purchase_invoice_id <> '') {	// entered a so/po/invoice value, check for dups
			...
			$result = $admin->DataBase->query($sql);
			if ($result->rowCount() > 0) throw new \core\classes\userException(sprintf(TEXT_THE_YOU_ENTERED_IS_A_DUPLICATE,_PLEASE_ENTER_A_NEW_UNIQUE_VALUE_ARGS, $journal_types_list[14]['id_field_name']));
