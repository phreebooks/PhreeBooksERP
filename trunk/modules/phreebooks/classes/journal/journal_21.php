...
// +-----------------------------------------------------------------+
//  Path: /modules/phreebooks/classes/journal/journal_21.php
//
// POP Journal (21)
...
class journal_21 extends \core\classes\journal {
	...
	public $closed 				= '0';
	public $gl_type             = 'por';
	public $popup_form_type		= 'vend:rcpt';
	...
	public $drop_ship           = 0;
	public $freight             = 0;
	public $description 		= TEXT_POINT_OF_PURCHASE;
	...
	function validate_purchase_invoice_id() {
		...
		if ($this->purchase_invoice_id <> '') {	// entered a so/po/invoice value, check for dups
			...
			$result = $admin->DataBase->query($sql);
			if ($result->fetch(\PDO::FETCH_NUM) > 0) throw new \core\classes\userException(sprintf(TEXT_THE_YOU_ENTERED_IS_A_DUPLICATE,_PLEASE_ENTER_A_NEW_UNIQUE_VALUE_ARGS, $journal_types_list[21]['id_field_name']));
			...
			if (!$result) throw new \core\classes\userException(sprintf(GL_ERROR_CANNOT_FIND_NEXT_ID, TABLE_CURRENT_STATUS));//@todo
