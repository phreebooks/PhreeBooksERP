...
class journal_20 extends \core\classes\journal {
	...
	public $error_6				= GENERAL_JOURNAL_20_ERROR_6;
	public $description 		= TEXT_VENDOR_PAYMENTS;
	...
	function validate_purchase_invoice_id() {
		...
		if ($this->purchase_invoice_id <> '') {	// entered a so/po/invoice value, check for dups
			$sql = "SELECT purchase_invoice_id FROM " . TABLE_JOURNAL_MAIN . " WHERE purchase_invoice_id = '{$this->purchase_invoice_id}' and journal_id = '20'";
			if ($this->id) $sql .= " and id <> " . $this->id;
			$result = $admin->DataBase->query($sql);
			if ($result->fetch(\PDO::FETCH_NUM) > 0) throw new \core\classes\userException(sprintf(TEXT_THE_YOU_ENTERED_IS_A_DUPLICATE,_PLEASE_ENTER_A_NEW_UNIQUE_VALUE_ARGS, $journal_types_list[20]['id_field_name']));
	...
	function increment_purchase_invoice_id($force = false) {
		...
		if ($this->purchase_invoice_id == '' || $force) { // increment the po/so/invoice number
			...
			$result = $admin->DataBase->exec($sql);
			if ($result->AffectedRows() <> 1) throw new \core\classes\userException(sprintf(TEXT_THERE_WAS_AN_ERROR_INCREMENTING_THE_ARGS, $journal_types_list[20]['id_field_name']));
		}
	...
	function post_ordr($action) {
		...
		// ***************************** END TRANSACTION *******************************
		$messageStack->add(sprintf(TEXT_SUCCESSFULLY_ARGS, TEXT_POSTED, $journal_types_list[20]['id_field_name'], $this->purchase_invoice_id), 'success');
		return true;
	}
	...
	function delete_payment() {
		...
		// *************** END TRANSACTION *************************
		$messageStack->add(sprintf(TEXT_SUCCESSFULLY_ARGS, TEXT_DELETED, $journal_types_list[20]['id_field_name'], $this->purchase_invoice_id), 'success');
		return true;
	}
	...
	function add_total_journal_row() {	// put total value into ledger row array
		$this->journal_rows[] = array( // record for accounts receivable
				...
				'description'   => $journal_types_list[20]['text'] . '-' . TEXT_TOTAL,
