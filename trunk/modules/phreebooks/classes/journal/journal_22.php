...
// +-----------------------------------------------------------------+
//  Path: /modules/phreebooks/classes/journal/journal_22.php
//
// customer Cash Receipts Journal (22) for vendor payments see 18
namespace phreebooks\classes\journal;
class journal_22 extends \core\classes\journal {
	...
	function validate_purchase_invoice_id() {
		...
		if ($this->purchase_invoice_id <> '') {	// entered a so/po/invoice value, check for dups
			...
			$result = $admin->DataBase->query("SELECT next_deposit_num FROM " . TABLE_CURRENT_STATUS . " LIMIT 1");
			if (!$result) throw new \core\classes\userException(sprintf(GL_ERROR_CANNOT_FIND_NEXT_ID, TABLE_CURRENT_STATUS));
			$this->journal_main_array['purchase_invoice_id'] = $result['next_deposit_num'];
	...
	function increment_purchase_invoice_id($force = false) {
		...
		if ($this->purchase_invoice_id == '' || $force) { // increment the po/so/invoice number
			...
			$result = $admin->DataBase->exec($sql);
			if ($result->AffectedRows() <> 1) throw new \core\classes\userException(sprintf(TEXT_THERE_WAS_AN_ERROR_INCREMENTING_THE_ARGS, $journal_types_list[22]['id_field_name']));
		}
	...
	function post_ordr($action) {
		...
		// ***************************** END TRANSACTION *******************************
		$messageStack->add(sprintf(TEXT_SUCCESSFULLY_ARGS, TEXT_POSTED, $journal_types_list[22]['id_field_name'], $this->purchase_invoice_id), 'success');
		return true;
	}
	...
	function bulk_pay() {
		...
		// don't force increment if it's a bulk receipt
		$this->increment_purchase_invoice_id(false);
		return true;
	}
	...
	function delete_payment() {
		...
		// *************** END TRANSACTION *************************
		$messageStack->add(sprintf(TEXT_SUCCESSFULLY_ARGS, TEXT_DELETED, $journal_types_list[22]['id_field_name'], $this->purchase_invoice_id), 'success');
		return true;
	}
	...
	function add_total_journal_row() {	// put total value into ledger row array
		...
		$this->journal_rows[] = array( // record for accounts receivable
				...
				'description'  => $journal_types_list[22]['text'] . '-' . TEXT_TOTAL . ':' . $admin->classes['payment']->methods[$method]->payment_fields, //@todo $method defined outside of this method
