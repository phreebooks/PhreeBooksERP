...
// +-----------------------------------------------------------------+
//  Path: /modules/phreebooks/classes/journal/journal_13.php
//
// Customer Credit Memo Journal (13)
namespace phreebooks\classes\journal;
class journal_13 extends \core\classes\journal {
	public $id;
	public $recur_id;
	...
	public $closed 				= false;
	public $gl_type             = 'sos';
	public $popup_form_type		= 'cust:cm';
	...
	public $text_column_1_title	= TEXT_SHIPPED;
	public $text_column_2_title	= TEXT_RETURNED;
	public $text_order_closed	= TEXT_CREDIT_PAID;
	...
	public $error_6 			= GENERAL_JOURNAL_13_ERROR_6;
	public $description 		= TEXT_CUSTOMER_CREDIT_MEMOS;
	...
	function calculate_COGS($item, $return_cogs = false) {
		...
		if ($item['qty'] > 0) { // for positive quantities, inventory received, customer credit memos, unbuild assembly
			...
			// update will never happen because the entries are removed during the unpost operation.
			 // not the invoice price (customers price)
	...
	function validate_purchase_invoice_id() {
		...
		if ($this->purchase_invoice_id <> '') {	// entered a so/po/invoice value, check for dups
			$sql = "SELECT purchase_invoice_id FROM " . TABLE_JOURNAL_MAIN . " WHERE purchase_invoice_id = '{$this->purchase_invoice_id}' and journal_id = '13'";
			if ($this->id) $sql .= " and id <> " . $this->id;
			$result = $admin->DataBase->query($sql);
			if ($result->fetch(\PDO::FETCH_NUM) > 0) throw new \core\classes\userException(sprintf(TEXT_THE_YOU_ENTERED_IS_A_DUPLICATE,_PLEASE_ENTER_A_NEW_UNIQUE_VALUE_ARGS, $journal_types_list[13]['id_field_name']));
			...
			$result = $admin->DataBase->query("SELECT next_cm_num FROM " . TABLE_CURRENT_STATUS . " LIMIT 1");
			if (!$result) throw new \core\classes\userException(sprintf(GL_ERROR_CANNOT_FIND_NEXT_ID, TABLE_CURRENT_STATUS));
			$this->journal_main_array['purchase_invoice_id'] = $result['next_cm_num'];
	...
	function increment_purchase_invoice_id($force = false) {
		...
		if ($this->purchase_invoice_id == '' || $force) { // increment the po/so/invoice number
			...
			$sql = "UPDATE " . TABLE_CURRENT_STATUS . " SET next_cm_num = '$next_id'";
			if (!$force) $sql .= " WHERE next_cm_num = '{$this->journal_main_array['purchase_invoice_id']}'";
			$result = $admin->DataBase->exec($sql);
			if ($result->AffectedRows() <> 1) throw new \core\classes\userException(sprintf(TEXT_THERE_WAS_AN_ERROR_INCREMENTING_THE_ARGS, $journal_types_list[13]['id_field_name']));
		}
	...
	function post_ordr($action) {
		...
		$debit_total  += $this->add_freight_journal_row();	// put freight into journal row array
		...
		// ***************************** END TRANSACTION *******************************
		$messageStack->add(sprintf(TEXT_SUCCESSFULLY_ARGS, TEXT_POSTED, $journal_types_list[13]['id_field_name'], $this->purchase_invoice_id), 'success');
		return true;
	}
	...
	function add_total_journal_row() {	// put total value into ledger row array
		$this->journal_rows[] = array( // record for accounts receivable
				...
				'description'   => $journal_types_list[13]['text'] . ' - ' . TEXT_TOTAL,
	...
	function add_discount_journal_row() { // put discount into journal row array
		if ($this->discount <> 0) {
			$this->journal_rows[] = array(
					...
					'description'   => $journal_types_list[13]['text'] . ' - ' . TEXT_DISCOUNT,
	...
	function add_freight_journal_row() {	// put freight into journal row array
		...
		if ($this->freight) { // calculate freight charges
			$this->journal_rows[] = array(
					...
					'description'  => $journal_types_list[13]['text'] . ' - ' . TEXT_SHIPPING,
