...
// +-----------------------------------------------------------------+
//  Path: /modules/phreebooks/classes/journal/journal_4.php
//
...
class journal_04 extends \core\classes\journal {
	...
	public $description 		= TEXT_PURCHASE_ORDERS;
	public $id_field_name 		= TEXT_PO_NUMBER;
	...
	function validate_purchase_invoice_id() {
		...
		if ($this->purchase_invoice_id <> '') {	// entered a so/po/invoice value, check for dups
			$sql = "SELECT purchase_invoice_id FROM " . TABLE_JOURNAL_MAIN . " WHERE purchase_invoice_id = '{$this->purchase_invoice_id}' and journal_id = '4'";
			if ($this->id) $sql .= " and id <> " . $this->id;
			$result = $admin->DataBase->query($sql);
			if ($result->fetch(\PDO::FETCH_NUM) > 0) throw new \core\classes\userException(sprintf(TEXT_THE_YOU_ENTERED_IS_A_DUPLICATE,_PLEASE_ENTER_A_NEW_UNIQUE_VALUE_ARGS, $journal_types_list[4]['id_field_name']));
			...
			$result = $admin->DataBase->query("SELECT next_po_num FROM " . TABLE_CURRENT_STATUS . " LIMIT 1");
			if (!$result) throw new \core\classes\userException(sprintf(GL_ERROR_CANNOT_FIND_NEXT_ID, TABLE_CURRENT_STATUS));
			$this->journal_main_array['purchase_invoice_id'] = $result['next_po_num'];
	...
	function increment_purchase_invoice_id($force = false) {
		...
		if ($this->purchase_invoice_id == '' || $force) { // increment the po/so/invoice number
			...
			$sql = "UPDATE " . TABLE_CURRENT_STATUS . " SET next_po_num = '$next_id'";
			if (!$force) $sql .= " WHERE next_po_num = '{$this->journal_main_array['purchase_invoice_id']}'";
			$result = $admin->DataBase->exec($sql);
			if ($result->AffectedRows() <> 1) throw new \core\classes\userException(sprintf(TEXT_THERE_WAS_AN_ERROR_INCREMENTING_THE_ARGS, $journal_types_list[4]['id_field_name']));
		}
	...
	function post_ordr($action) {
		...
		if ($this->recur_id > 0) { // if new record, will contain count, if edit will contain recur_id
			...
			if ($this->id) { // it's an edit, fetch list of affected records to update if roll is enabled
				...
				for ($i = 0; $i < $this->recur_id; $i++) {
					...
					$this->journal_main_array['period']        = $this->period;
					$this->journal_main_array['terminal_date'] = $this->terminal_date;
					if ($first_purchase_invoice_id == '') $this->increment_purchase_invoice_id(true);
			...
			if ($first_purchase_invoice_id == '') { // special case for auto increment
				$first_purchase_invoice_id = $this->purchase_invoice_id;
			}
			$this->id                  = $first_id;
			$this->post_date           = $first_post_date;
		...
		// ***************************** END TRANSACTION *******************************
		$messageStack->add(sprintf(TEXT_SUCCESSFULLY_ARGS, TEXT_POSTED, $journal_types_list[4]['id_field_name'], $this->purchase_invoice_id), 'success');
		return true;
	}
	...
	function add_total_journal_row() {	// put total value into ledger row array
		$this->journal_rows[] = array( // record for accounts receivable
				...
				'description'   => $journal_types_list[4]['text'] . ' - ' . TEXT_TOTAL,
	...
	function add_discount_journal_row() { // put discount into journal row array
		if ($this->discount <> 0) {
			$this->journal_rows[] = array(
					...
					'description'             => $journal_types_list[4]['text'] . ' - ' . TEXT_DISCOUNT,
	...
	function add_freight_journal_row() {	// put freight into journal row array
		...
		if ($this->freight) { // calculate freight charges
			$this->journal_rows[] = array(
					...
					'description'  => $journal_types_list[4]['text'] . ' - ' . TEXT_SHIPPING,
		...
		for ($i=0; $i<count($this->item_rows); $i++) {
			if ($this->item_rows[$i]['qty']) { // make sure the quantity line is set and not zero
				$this->journal_rows[] = array(
						...
						'post_date'               => $this->post_date,
						'date_1'                  => $this->item_rows[$i]['date_1'] ? $this->item_rows[$i]['date_1'] : gen_specific_date($this->post_date, $this->item_rows[$i]['lead_time']),
				);
