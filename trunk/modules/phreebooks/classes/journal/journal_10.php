...
// +-----------------------------------------------------------------+
//  Path: /modules/phreebooks/classes/journal/journal_10.php
//
// Sales Order Journal (10)
namespace phreebooks\classes\journal;
class journal_10 extends \core\classes\journal {
	public $id;
	public $recur_id;
	...
	public $closed 				= false;
	public $gl_type             = 'soo';
	public $popup_form_type		= 'cust:so';
	...
	public $error_6 			= GENERAL_JOURNAL_10_ERROR_6;
	public $description 		= TEXT_SALES_ORDERS;
	public $id_field_name 		= TEXT_SO_NUMBER;
	...
	function unPost_inventory() {
		...
		for ($i = 0; $i < count($this->journal_rows); $i++) if ($this->journal_rows[$i]['sku']) {
			...
			if (!$this->closed && $bal_before_post > 0) $this->update_inventory_status($this->journal_rows[$i]['sku'], 'quantity_on_sales_order', -$bal_before_post);
		}
	...
	function load_so_po_balance($ref_id, $id = '', $post = true) {
		...
		if ($ref_id) {
			...
			WHERE m.so_po_ref_id = {$ref_id} and i.gl_type = 'sos'";
	...
	function check_for_closed_po_so($action = 'Post') {
		...
		for ($i = 0; $i < count($this->journal_rows); $i++) {
			if ($this->journal_rows[$i]['qty'] && $this->journal_rows[$i]['gl_type'] == 'soo') $item_rows_all_zero = false; // at least one qty is non-zero
		}
	...
	function validate_purchase_invoice_id() {
		...
		if ($this->purchase_invoice_id <> '') {	// entered a so/po/invoice value, check for dups
			$sql = "SELECT purchase_invoice_id FROM " . TABLE_JOURNAL_MAIN . " WHERE purchase_invoice_id = '{$this->purchase_invoice_id}' and journal_id = '10'";
			if ($this->id) $sql .= " and id <> " . $this->id;
			$result = $admin->DataBase->query($sql);
			if ($result->fetch(\PDO::FETCH_NUM) > 0) throw new \core\classes\userException(sprintf(TEXT_THE_YOU_ENTERED_IS_A_DUPLICATE,_PLEASE_ENTER_A_NEW_UNIQUE_VALUE_ARGS, $journal_types_list[10]['id_field_name']));
	...
	function post_ordr($action) {
		...
		if ($this->recur_id > 0) { // if new record, will contain count, if edit will contain recur_id
			...
			if ($first_purchase_invoice_id == '') $first_purchase_invoice_id = $this->purchase_invoice_id;
			$this->id                  = $first_id;
			$this->post_date           = $first_post_date;
			$this->purchase_invoice_id = $first_purchase_invoice_id;
			$this->terminal_date       = $first_terminal_date;
		...
		// ***************************** END TRANSACTION *******************************
		$messageStack->add(sprintf(TEXT_SUCCESSFULLY_ARGS, TEXT_POSTED, $journal_types_list[10]['id_field_name'], $this->purchase_invoice_id), 'success');
		return true;
	}
	...
	function add_total_journal_row() {	// put total value into ledger row array
		$this->journal_rows[] = array( // record for accounts receivable
				...
				'description'  => $journal_types_list[10]['text'] . ' - ' . TEXT_TOTAL,
	...
	function add_discount_journal_row() { // put discount into journal row array
		if ($this->discount <> 0) {
			$this->journal_rows[] = array(
					...
					'description'  => $journal_types_list[10]['text'] . ' - ' . TEXT_DISCOUNT,
	...
	function add_freight_journal_row() {	// put freight into journal row array
		...
		if ($this->freight) { // calculate freight charges
			$this->journal_rows[] = array(
					...
					'description'   => $journal_types_list[10]['text'] . ' - ' . TEXT_SHIPPING,
