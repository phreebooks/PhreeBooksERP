<?php
// +-----------------------------------------------------------------+
// Phreedom Language Translation File
// Generated: 2011-11-27 05:02:34
// Module/Method: phreebooks
// ISO Language: nl_nl
// Version: 3.3
// +-----------------------------------------------------------------+
// Path: /modules/phreebooks/language/nl_nl/menu.php

define('TEXT_GENERAL_LEDGER','Boekhouden');
define('TEXT_BANKING','Bankieren');
define('ORD_TEXT_0_WINDOW_TITLE','Begin Balans');
define('TEXT_GENERAL_JOURNAL','Journaal');
define('TEXT_REQUEST_FOR_QUOTES','Inkoop Verzoek');
define('ORD_TEXT_4_WINDOW_TITLE','Inkoop Order');
define('TEXT_PURCHASE_OR_RECEIVE_INVENTORY','Inkoop Factuur');
define('ORD_TEXT_7_WINDOW_TITLE','Inkoop Credit');
define('TEXT_SALES_QUOTES','Verkoop Offerte');
define('ORD_TEXT_10_WINDOW_TITLE','Verkoop Order');
define('TEXT_SALES_INVOICES','Verkoop Factuur');
define('ORD_TEXT_13_WINDOW_TITLE','Verkoop Credit');
define('ORD_TEXT_18_WINDOW_TITLE','Kas ontvangsten');
define('ORD_TEXT_20_WINDOW_TITLE','Kas Distributie');
define('ORD_TEXT_18_C_WINDOW_TITLE','Klant Betalingen');
define('ORD_TEXT_18_V_WINDOW_TITLE','Restituties');
define('ORD_TEXT_19_WINDOW_TITLE','Kassa');
define('TEXT_CUSTOMER_REFUNDS','Klant Restituties');
define('ORD_TEXT_20_V_WINDOW_TITLE','Leverancier Betalingen');
define('TEXT_PAY_BY_DUE_DATE','Betaal per Verval Datum');
define('TEXT_BANK_ACCOUNT_REGISTER','Bank Rekening Register');
define('BOX_BANKING_ACCOUNT_RECONCILIATION','Grootboek Verrekening');
define('TEXT_VOID_CHECKS','Ingetrokken Checks');
define('TEXT_BUDGETING','Budgeteren');
define('TEXT_GENERAL_JOURNAL_UTILITIES','Algemene Journaal Opties');
define('TEXT_PHREEBOOKS_ADMINISTRATION','PhreeBooks Admin');
define('TEXT_MANAGER_ARGS','%s Manager');
define('ORD_TEXT_RECEIPTS_TITLE','Ontvangsten');
define('ORD_TEXT_PAYMENTS_TITLE','Betalingen');
define('TEXT_QUOTES','Offertes');
define('TEXT_NEW_ARGS','Nieuwe %s');
define('TEXT_ORDERS','Orders');
define('ORD_TEXT_INVOICE_TITLE','Facturen');
define('TEXT_CREDIT','Credit');
?>
