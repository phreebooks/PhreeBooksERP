  	...
  	try{
		...
		for ($i = 0; $i < sizeof($temp_rows); $i++) {
			if ($temp_rows[$i]['gl_type'] <> 'soo') continue; // remove all rows except valid order lines
		  	...
		  	if ($temp_rows[$i]['sku']) {
		    	$result = $admin->DataBase->query("select description_purchase, item_cost, account_inventory_wage, vendor_id
		...
		$result = $admin->DataBase->query("select * from " . TABLE_ADDRESS_BOOK . " where ref_id = " . $vendor_id . " and type = 'vm'");
		...
		$result = $admin->DataBase->query("select special_terms from " . TABLE_CONTACTS . " where id = " . $vendor_id);
