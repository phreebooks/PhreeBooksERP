...
switch ($_REQUEST['action']) {
	case 'toggle' :
		...
		$result = $admin->DataBase->query ( "select waiting from " . TABLE_JOURNAL_MAIN . " where id = '$id'" );
		$toggle = $result->fields ['waiting'] ? '0' : '1';
		$admin->DataBase->query ( "update " . TABLE_JOURNAL_MAIN . " set waiting = '$toggle' where id = '$id'" );
		break;
	case 'dn_attach' :
