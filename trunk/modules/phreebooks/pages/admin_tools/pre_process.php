...
if ($_POST['fy']) {
  ...
  $result = $admin->DataBase->query("select fiscal_year from " . TABLE_ACCOUNTING_PERIODS . " where period = " . CURRENT_ACCOUNTING_PERIOD);
  $fy = $result->fields['fiscal_year'];
}
// find the highest posted period to disallow accounting period changes
$result     = $admin->DataBase->query("select max(period) as period from " . TABLE_JOURNAL_MAIN);
$max_period = ($result->fields['period'] > 0) ? $result->fields['period'] : 0;
// find the highest fiscal year and period in the system
$result     = $admin->DataBase->query("select max(fiscal_year) as fiscal_year, max(period) as period from " . TABLE_ACCOUNTING_PERIODS);
...
switch ($_REQUEST['action']) {
  ...
  case 'new':
	\core\classes\user::validate_security($security_level, 2);
  	$result = $admin->DataBase->query("select * from " . TABLE_ACCOUNTING_PERIODS . " where period = " . $highest_period);
  ...
  case "change":
	...
	$admin->DataBase->query("update " . TABLE_CONFIGURATION . " set configuration_value = $period
		where configuration_key = 'CURRENT_ACCOUNTING_PERIOD'");
	$admin->DataBase->query("update " . TABLE_CONFIGURATION . " set configuration_value = '{$result->fields['start_date']}'
		where configuration_key = 'CURRENT_ACCOUNTING_PERIOD_START'");
	$admin->DataBase->query("update " . TABLE_CONFIGURATION . " set configuration_value = '{$result->fields['end_date']}'
  ...
  case 'purge_db':
	\core\classes\user::validate_security($security_level, 4);
  	if ($_POST['purge_confirm'] == 'purge') {
		$admin->DataBase->query("TRUNCATE TABLE " . TABLE_JOURNAL_MAIN);
		$admin->DataBase->query("TRUNCATE TABLE " . TABLE_JOURNAL_ITEM);
		$admin->DataBase->query("TRUNCATE TABLE " . TABLE_ACCOUNTS_HISTORY);
		$admin->DataBase->query("TRUNCATE TABLE " . TABLE_INVENTORY_HISTORY);
		$admin->DataBase->query("TRUNCATE TABLE " . TABLE_INVENTORY_COGS_OWED);
		$admin->DataBase->query("TRUNCATE TABLE " . TABLE_INVENTORY_COGS_USAGE);
		$admin->DataBase->query("TRUNCATE TABLE " . TABLE_RECONCILIATION);
  ...
  case 'coa_hist_fix':
	...
	$history = array();
	$bad_accounts = array();
	$result = $admin->DataBase->query("select period, account_id, beginning_balance, debit_amount, credit_amount
	...
	foreach ($history as $acct => $activity) {
	  foreach ($activity as $period => $data) {
		...
		$posted = $admin->DataBase->query("select sum(debit_amount) as debit, sum(credit_amount) as credit
...
$cal_end   = array();
$i = 0;
$result = $admin->DataBase->query("select period, start_date, end_date from ".TABLE_ACCOUNTING_PERIODS." where fiscal_year = $fy");
while(!$result->EOF) {
