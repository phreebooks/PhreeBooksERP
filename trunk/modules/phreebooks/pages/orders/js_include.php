...
var text_terms           = '<?php echo gen_terms_to_language('0', true, ($order->account_type == "v") ? 'AP' : 'AR'); ?>';
var text_Please_Select   = '<?php echo TEXT_PLEASE_SELECT; ?>';
...
var default_freight_acct = '<?php echo ($order->account_type == "v") ? AP_DEF_FREIGHT_ACCT : AR_DEF_FREIGHT_ACCT; ?>';
...
var item_col_1_enable    = '<?php echo ($order->item_col_1_enable) ? '1' : '0'; ?>';
var item_col_2_enable    = '<?php echo ($order->item_col_2_enable) ? '1' : '0'; ?>';
var ship_search_HTML     = '<?php echo TEXT_CUSTOMER_ID . ": " . html_input_field("ship_search", $order->ship_short_name) . "&nbsp;" . html_icon("status/folder-open.png", TEXT_SEARCH, "small", 'align="top" style="cursor:pointer" title="' . TEXT_SEARCH . '" onclick="DropShipList(this)"'); ?>';
...
var tax_freight          = '<?php echo ($order->account_type == "c") ? AR_ADD_SALES_TAX_TO_SHIPPING : AP_ADD_SALES_TAX_TO_SHIPPING; ?>';
...
var lowStockProcessed    = '<?php echo TEXT_PROCESED ?> ';
var lowStockNoVendor     = '<?php echo TEXT_NO_VENDOR_FOUND?>';
...
var ItemIsAlreadyOnOrder = '<?php echo TEXT_ITEM_IS_ALREADY_ON_ORDER; ?>';
...
<?php
  	...
  	if (!$error && $_REQUEST['action'] == 'print') {//@todo
		...
		echo '  ClearForm();' . "\n";
		echo '  var printWin = window.open("index.php?module=phreeform&page=popup_gen&gID=' . $order->popup_form_type . '&date=a&xfld=journal_main.id&xcr=EQUAL&xmin=' . $order->id . '","popup_gen","width=700px,height=550px,resizable=1,scrollbars=1,top=150px,left=200px");' . "\n";
    	echo '  printWin.focus();' . "\n";
  	}
