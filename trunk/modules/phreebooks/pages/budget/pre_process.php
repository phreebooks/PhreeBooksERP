...
switch ($_REQUEST['action']) {
  ...
  case 'clear_fy':
	$result  = $admin->DataBase->query("select period from " . TABLE_ACCOUNTING_PERIODS . " where fiscal_year = '" . $fy . "'");
	$periods = array();
	while (!$result->EOF) {
  ...
  case 'copy_fy':
	...
	$last_fy = $admin->DataBase->query("select period from " . TABLE_ACCOUNTING_PERIODS . "
	  where fiscal_year = '" . ($fy - 1) . "' order by period");
	while (!$result->EOF) {
	  $accounts = $admin->DataBase->query("select account_id, debit_amount - credit_amount as balance
...
$last_fy = $admin->DataBase->query("select period from " . TABLE_ACCOUNTING_PERIODS . "
	where fiscal_year = '" . ($fy - 1) . "' order by period");
$next_fy = $admin->DataBase->query("select period from " . TABLE_ACCOUNTING_PERIODS . "
	where fiscal_year = '" . ($fy + 1) . "' order by period");
$periods = array();
$lf_per = array();
$nf_per = array();
...
$result = $admin->DataBase->query("select id, period, budget from " . TABLE_CHART_OF_ACCOUNTS_HISTORY . "
	where account_id = '" . $gl_acct . "' and period in (" . implode(',', array_keys($periods)) . ")");
if ($gl_acct && sizeof($lf_per) > 0) {
  $last_fy = $admin->DataBase->query("select budget from " . TABLE_CHART_OF_ACCOUNTS_HISTORY . "
...
if ($gl_acct && sizeof($nf_per) > 0) {
  $next_fy = $admin->DataBase->query("select budget from " . TABLE_CHART_OF_ACCOUNTS_HISTORY . "
...
define('PAGE_TITLE', sprintf(TEXT_MANAGER_ARGS, TEXT_BUDGET));
