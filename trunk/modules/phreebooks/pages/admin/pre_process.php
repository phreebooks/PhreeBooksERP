...
switch ($_REQUEST['action']) {
  case 'import':
	...
	$filename = $std_chart ? (DIR_FS_WORKING.'language/'.$_SESSION['user']->language.'/charts/'.$std_chart) : $_FILES['file_name']['tmp_name'];
...
if (is_dir($dir = DIR_FS_WORKING."language/{$_SESSION['user']->language}/charts")) {
	$charts = @scandir($dir);
...
foreach ($charts as $chart) {
  	if (strpos($chart, 'xml')) {
  		if (($file = @file_get_contents(DIR_FS_WORKING . "language/{$_SESSION['user']->language}/charts/$chart")) === false)  throw new \core\classes\userException(sprintf(ERROR_READ_FILE, DIR_FS_WORKING . "language/{$_SESSION['user']->language}/charts/$chart"));
...
define('PAGE_TITLE', TEXT_PHREEBOOKS_ADMINISTRATION);
