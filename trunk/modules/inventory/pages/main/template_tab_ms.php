    	...
    	$odd = false;
    	 if(!empty($basis->cInfo->inventory->child_array)){
    		foreach ($basis->cInfo->inventory->child_array as $value) {
				...
				echo '<td width="20px" align="right">' . $basis->currencies->precise($value['cost']) . '</td>'. chr(10);
				echo '<td width="20px" align="right">' . $basis->currencies->precise($value['price']) . '</td>'. chr(10);
		      	echo '</tr>' . chr(10);
		      	$odd = !$odd;
    		}
    	} ?>
