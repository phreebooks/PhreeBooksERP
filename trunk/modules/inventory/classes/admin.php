...
define ( 'TABLE_INVENTORY_CONNECTIONS', DB_PREFIX . 'inventory_connections' );
...
;
	...
	function __construct() {
		...
		$this->tables = array (
			...
			TABLE_INVENTORY_CONNECTIONS => "CREATE TABLE " . TABLE_INVENTORY_CONNECTIONS . " ( 
				sku_1 varchar(24) NOT NULL default '',
				sku_2 varchar(24) NOT NULL default '',
	...
	function install($path_my_files, $demo = false) {
		...
		while ( $result = $sql->fetch ( \PDO::FETCH_ASSOC ) ) {
			...
			if (in_array ( $result ['field_name'], $haystack ))
	...
	function upgrade(\core\classes\basis &$basis) {
		...
		if (version_compare ( $this->status, '3.2', '<' )) {
			if (! $basis->DataBase->field_exists ( TABLE_PRICE_SHEETS, 'type' ))
				$basis->DataBase->query ( "ALTER TABLE " . TABLE_PRICE_SHEETS . " ADD type char(1) NOT NULL default 'c' AFTER sheet_name" );
			if (! $basis->DataBase->field_exists ( TABLE_INVENTORY, 'price_sheet_v' ))
				$basis->DataBase->query ( "ALTER TABLE " . TABLE_INVENTORY . " ADD price_sheet_v varchar(32) default NULL AFTER price_sheet" );
		}
		if (version_compare ( $this->status, '3.6', '<' )) {
			$basis->DataBase->query ( "ALTER TABLE " . TABLE_INVENTORY . " ADD INDEX ( `sku` )" );
			if (! $basis->DataBase->field_exists ( TABLE_INVENTORY, 'attachments' ))
				$basis->DataBase->query ( "ALTER TABLE " . TABLE_INVENTORY . " ADD attachments text AFTER last_journal_date" );
			if (! $basis->DataBase->field_exists ( TABLE_INVENTORY, 'full_price_with_tax' ))
				$basis->DataBase->query ( "ALTER TABLE " . TABLE_INVENTORY . " ADD full_price_with_tax FLOAT NOT NULL DEFAULT '0' AFTER full_price" );
			if (! $basis->DataBase->field_exists ( TABLE_INVENTORY, 'product_margin' ))
				$basis->DataBase->query ( "ALTER TABLE " . TABLE_INVENTORY . " ADD product_margin FLOAT NOT NULL DEFAULT '0' AFTER full_price_with_tax" );
			if (! $basis->DataBase->field_exists ( TABLE_EXTRA_FIELDS, 'use_in_inventory_filter' ))
				$basis->DataBase->query ( "ALTER TABLE " . TABLE_EXTRA_FIELDS . " ADD use_in_inventory_filter ENUM( '0', '1' ) NOT NULL DEFAULT '0'" );
			...
			while ( $result = $sql->fetch ( \PDO::FETCH_ASSOC ) ) {
				...
				if ($result ['item_cost'] != '' && $result ['item_cost'] > 0)
					$sql_data_array ['product_margin'] = round ( $sql_data_array ['full_price_with_tax'] / $result ['item_cost'], $basis->currencies->currencies [DEFAULT_CURRENCY] ['decimal_places'] );
		...
		if (version_compare ( $this->status, '4.0', '<' )) {
			if (! $basis->DataBase->field_exists ( TABLE_INVENTORY, 'class' ))
				$basis->DataBase->exec ( "ALTER TABLE " . TABLE_INVENTORY . " ADD class VARCHAR( 255 ) NOT NULL DEFAULT '' FIRST" );
			$basis->DataBase->exec ( "UPDATE " . TABLE_INVENTORY . " SET class = CONCAT('inventory\\\\classes\\\\type\\\\', inventory_type) WHERE class = '' " );
			if ($basis->DataBase->field_exists ( TABLE_INVENTORY, 'description_purchase' ))
				$basis->DataBase->exec ( "ALTER TABLE " . TABLE_INVENTORY . " DROP description_purchase" );
			if ($basis->DataBase->field_exists ( TABLE_INVENTORY, 'purch_taxable' ))
				$basis->DataBase->exec ( "ALTER TABLE " . TABLE_INVENTORY . " DROP purch_taxable" );
			if ($basis->DataBase->field_exists ( TABLE_INVENTORY, 'item_cost' ))
				$basis->DataBase->exec ( "ALTER TABLE " . TABLE_INVENTORY . " DROP item_cost" );
			if ($basis->DataBase->field_exists ( TABLE_INVENTORY, 'price_sheet_v' ))
				$basis->DataBase->exec ( "ALTER TABLE " . TABLE_INVENTORY . " DROP price_sheet_v" );
	...
	function validate_name($sku) {
		global $admin;
		\core\classes\messageStack::development ( "executing " . __METHOD__ . " checking sku = $sku" );
		if (! $sku)
			throw new \core\classes\userException ( TEXT_THE_ID_FIELD_WAS_EMPTY );
		$sql = $admin->DataBase->prepare ( "SELECT COUNT(*) FROM " . TABLE_INVENTORY . " WHERE sku = :sku" );
		$sql->execute ( array (
				':sku' => $sku 
		) );
		if ($sql->fetchColumn () != 0)
			throw new \core\classes\userException ( sprintf ( TEXT_THE_SKU_IS_NOT_UNIQUE_ARGS, $sku ) );
		return true;
	}
	...
	function DeleteInventoryItem(\core\classes\basis $basis) {
		...
		if (property_exists ( $basis->cInfo, 'inventory_id' ) !== true)
			throw new \core\classes\userException ( TEXT_ID_NOT_DEFINED );
		$sql = $basis->DataBase->prepare ( "SELECT * FROM " . TABLE_INVENTORY . " WHERE id = {$basis->cInfo->inventory_id}" );
		$sql->execute ();
		$basis->cInfo->inventory = $sql->fetch ( \PDO::FETCH_CLASS | \PDO::FETCH_CLASSTYPE );
		$basis->cInfo->inventory->check_remove ();
	...
	function RenameInventoryItem(\core\classes\basis $basis) {
		...
		$sql->execute ();
		$basis->cInfo->inventory = $sql->fetch ( \PDO::FETCH_CLASS | \PDO::FETCH_CLASSTYPE );
		$basis->cInfo->inventory->rename ( $basis->cInfo->new_sku );
		$basis->cInfo->success = true;
	...
	function CopyInventoryItem(\core\classes\basis $basis) {
		...
		$sql->execute ();
		$basis->cInfo->inventory = $sql->fetch ( \PDO::FETCH_CLASS | \PDO::FETCH_CLASSTYPE );
		$basis->cInfo->inventory->copy ( $basis->cInfo->new_sku );
		$basis->cInfo->success = true;
		$basis->cInfo->message = TEXT_COPIED_SUCCESSFULLY;
	}
	...
	function CreateInventoryItem(\core\classes\basis &$basis) {
		if (! isset ( $basis->cInfo->inventory_type ))
			throw new \core\classes\userException ( "inventory type isn't submitted." );
	...
	function editInventory(\core\classes\basis $basis) {
		...
		if (property_exists ( $basis->cInfo, 'rowSeq' ) === true)
			$basis->cInfo->inventoryID = $basis->cInfo->rowSeq;
		if (property_exists ( $basis->cInfo, 'inventoryID' ) !== true)
			throw new \core\classes\userException ( "inventoryID variable isn't set can't execute method editInventory " );
...
<div data-options="region:'center'">
	<table id="filter_table" class="ui-widget"
	...
	<table id="dg"
		title="&lt;?php echo sprintf(TEXT_MANAGER_ARGS, TEXT_INVENTORY);?&gt;"
		style="height: 500px; padding: 50px;">
		<thead>
			<tr>
	...
	<div id="toolbar">
		<a class="easyui-linkbutton" iconCls="icon-edit" plain="true"
			onclick="editInventory()"><?php echo sprintf(TEXT_EDIT_ARGS, TEXT_INVENTORY);?></a>
		<a class="easyui-linkbutton" iconCls="icon-add" plain="true"
			onclick="newInventory()"><?php echo sprintf(TEXT_NEW_ARGS, TEXT_INVENTORY);?></a>
		<a class="easyui-linkbutton" iconCls="icon-remove" plain="true"
			onclick="deleteInventory()"><?php echo sprintf(TEXT_DELETE_ARGS, TEXT_INVENTORY);?></a>
	...
	<div id="win" class="easyui-window">
		<div id="contactToolbar" style="margin: 2px 5px;">
			<a class="easyui-linkbutton" iconCls="icon-undo" plain="true"
				onclick="closeWindow()"><?php echo TEXT_CANCEL?></a>
  						<?php if (\core\classes\user::validate(SECURITY_ID_MAINTAIN_INVENTORY, true) < 2){?>
  						<a class="easyui-linkbutton" iconCls="icon-save" plain="true"
				onclick="saveContact()"><?php echo TEXT_SAVE?></a>
  						<?php }?>
  						<a class="easyui-linkbutton" iconCls="icon-help" plain="true"
				onclick="loadHelp()"><?php TEXT_HELP?></a>
		</div>
	</div>
</div>
<script type="text/javascript">
  				function actionformater (value,row,index){ 
  	  				var temp = '';
  	  				var security_level = <?php echo \core\classes\user::validate(SECURITY_ID_MAINTAIN_INVENTORY)?>;
  					if (security_level > 1) temp += buildIcon(icon_path+'16x16/actions/edit-find-replace.png', "<?php echo sprintf(TEXT_EDIT_ARGS, TEXT_INVENTORY);?>", 'onclick="editInventory('+row.id+')"');
  					if (security_level > 3 && row.inventory_type != 'mi' && row.inventory_type != 'ia') temp += buildIcon(icon_path+'16x16/apps/accessories-text-editor.png', '<?php echo TEXT_RENAME;?>', 'onclick="renameItem('+row.id+')"');
  					if (security_level > 3 && row.inventory_type != 'mi' && row.inventory_type != 'ia' && (row.last_journal_date != '0000-00-00 00:00:00' || row.last_journal_date != '')) temp += buildIcon(icon_path+'16x16/emblems/emblem-unreadable.png', "<?php echo TEXT_DELETE;?>", 'onclick="deleteItem('+row.id+')"');
  					if (security_level > 3 && row.inventory_type != 'mi' && row.inventory_type != 'ia') temp += buildIcon(icon_path+'16x16/actions/edit-copy.png', "<?php echo TEXT_COPY;?>", 'onclick="copyItem('+row.id+')"');
  					if (row.attachments != '')  temp += buildIcon(icon_path+'16x16/status/mail-attachment.png', "<?php echo TEXT_DOWNLOAD_ATTACHMENT;?>",'onclick="downloadAttachment('+row.id+')"'); //@todo
  					if (security_level > 2)  temp += buildIcon(icon_path+'16x16/mimetypes/x-office-spreadsheet.png', "<?php echo TEXT_CUSTOMER_PRICE_SHEETS;?>",'onclick="priceMgr('+row.id+','+row.full_price+',\'c\')"');//@TODO NEEDS TO BE ADDED
					return temp;
  				}
...
//  						document.location = "index.php?action=editInventory&inventoryID="+ row.id;
  				...
  				function deleteItem(id) {
  				    ...
  				    console.log('delete inventory item was clicked');
  				    $.messager.confirm('<?php echo TEXT_CONFORM?>','<?php echo sprintf(TEXT_ARE_YOU_SURE_YOU_WANT_TO_DELETE_ARGS, TEXT_INVENTORY_ITEM)?>',function(r){
  				    	if (r){
  				        	$.post('index.php?action=deleteInventoryItem',{inventory_id: id, dataType: 'json', async: false, contentType: 'application/json'},function(data){
  				        		...
  				        		if(data.success == false){
  				            		...
  				            		console.log('succesfully deleted inventory item '+ id);
  				...
  				function copyItem(id) {
  					var index = $('#dg').datagrid('getRowIndex', id);
  				    console.log('copy inventory item was clicked');
  				    $.messager.prompt('<?php echo TEXT_COPY;?>', '<?php echo TEXT_COPY_TO; ?>', function(newsku){
  				    	if (newsku){
  				        	$.post('index.php?action=CopyInventoryItem',{inventory_id: id, new_sku: newsku, dataType: 'json', async: false, contentType: 'application/json'},function(data){
  				        		console.log('result of copy '+ JSON.stringify(data));	
  				        		if(data.success == false){
  	  				        		$.messager.alert('<?php echo TEXT_ERROR?>',data.error_message);
  	  				        		console.error('error copy inventory item '+ id);
  				                }else{
  				            		data = eval('('+result+')');
  				            		console.log('succesfully copy inventory item '+ id);
  				            		$('#dg').datagrid('appendRow', data.inventory);
  				                }
  				            },'json')
  				          	.fail(function(xhr, status, error) {
  				          		console.error('we received a error from the server returned = '+ JSON.stringify(xhr));
  				...
  				function renameItem(id) {
  					var index = $('#dg').datagrid('getRowIndex', id);
  				    console.log('rename inventory item was clicked');
  				    $.messager.prompt('<?php echo TEXT_RENAME;?>', '<?php echo TEXT_RENAME_TO; ?>', function(newsku){
  				    	if (newsku){
  				        	$.post('index.php?action=RenameInventoryItem',{inventory_id: id, new_sku: newsku, dataType: 'json', async: false, contentType: 'application/json'},function(data){
  				        		console.log('result of rename '+ JSON.stringify(data));	
  				        		if(data.success == false){
  	  				        		$.messager.alert('<?php echo TEXT_ERROR?>',data.error_message);
  	  				        		console.error('error rename inventory item '+ id);
  				                }else{
  				            		console.log('succesfully rename inventory item '+ id);
				...
				function editInventory (id){
					window.open("index.php?action=editInventory&inventoryID="+ id,"editInventory","width=900,height=650,resizable=1,scrollbars=1,top=150,left=200");
  		        }
  			</script><?php
		$basis->observer->send_footer ( $basis );
	}
	function GetAllInventory(\core\classes\basis $basis) {
		\core\classes\messageStack::development ( "executing " . __METHOD__ );
		if ($basis->cInfo->rows == 'NaN')
			$basis->cInfo->rows = MAX_DISPLAY_SEARCH_RESULTS;
		...
		if (sizeof ( $basis->cInfo->filter ) > 0) {
			...
			foreach ( $basis->cInfo->filter as $key => $row ) {
				if ($filter_criteria [$row ['criteria']] == " LIKE " || $row ['criteria'] == FILTER_CONTAINS) {
					if ($row ['value'] != '')
						$criteria [] = "{$row['field']}  Like '%{$row['value']}%' ";
				} elseif ($filter_criteria [$row ['criteria']] == " NOT LIKE ") {
					if ($row ['value'] != '')
						$criteria [] = "{$row['field']} Not Like '%{$row['value']}%' ";
		...
		if ($basis->cInfo->q != '') $basis->cInfo->search_text = $basis->cInfo->q; 
		...
		if ($basis->cInfo->sort == 'sku')
			$basis->cInfo->sort = " LPAD(a.sku," . MAX_INVENTORY_SKU_LENGTH . ",0) ";
		...
		$sql = $basis->DataBase->prepare ( "SELECT DISTINCT a.* FROM " . TABLE_INVENTORY . " a LEFT JOIN " . TABLE_INVENTORY_PURCHASE . " p ON a.sku = p.sku {$search} ORDER BY {$basis->cInfo->sort} {$basis->cInfo->order} $offset" );
		$sql->execute ();
	...
	function before_LoadInventoryManager(\core\classes\basis $basis) {
		...
		$sql->execute ();
		while ( $result = $sql->fetch ( \PDO::FETCH_ASSOC ) ) {
			$temp = $result ['field_name'];
			if (in_array ( $temp, array (
					'vendor_id',
					'description_purchase',
					'item_cost',
					'purch_package_quantity',
					'purch_taxable',
					'price_sheet_v' 
			) )) {
				$field_name = "p.{$temp}";
			...
			$FirstId .= "FirstId[$i] = '{$field_name}';" . chr ( 10 );
			Switch ($temp) {
				case 'vendor_id' :
					...
					$tempValue .= '")';
					$tempId .= '")';
					$SecondField .= "SecondField['{$field_name}'] = 'drop_down';" . chr ( 10 );
					$SecondFieldValue .= "SecondFieldValue['{$field_name}'] = {$tempValue};" . chr ( 10 );
					$SecondFieldId .= "SecondFieldId['{$field_name}'] = {$tempId};" . chr ( 10 );
					break;
				...
				case 'cost_method' :
					...
					break;
				default :
					$SecondField .= "SecondField['{$field_name}'] ='{$result['entry_type']}';" . chr ( 10 );
					if (in_array ( $result ['entry_type'], array (
							'drop_down',
							'radio',
							'multi_check_box',
							'data_list' 
...
echo $FirstValue . chr ( 10 );
	...
	function getPurchasedetails(\core\classes\basis $basis) {
		\core\classes\messageStack::development ( "executing " . __METHOD__ );
		if (property_exists ( $basis->cInfo, 'sku' ) !== true)
			throw new \core\classes\userException ( "sku variable isn't set can't execute method getPurchasedetails " );
	...
	function SaveInventoryAdjustment(\core\classes\basis $basis) {
		...
		if ($adj_lines == 0)
			throw new \core\classes\userException ( TEXT_CANNOT_ADJUST_INVENTORY_WITH_A_ZERO_QUANTITY );
	...
	function SaveInventoryAssemblies(\core\classes\basis $basis) {
		...
		if (! $glEntry->period)
			throw new \core\classes\userException ( "period isn't set" );
		...
		if (! $result->fetch ( \PDO::FETCH_NUM ))
			throw new \core\classes\userException ( INV_ERROR_SKU_INVALID );
		if ($qty < 0 && ($result ['quantity_on_hand'] + $qty) < 0)
			throw new \core\classes\userException ( INV_ERROR_NEGATIVE_BALANCE );
		if (! $qty)
			throw new \core\classes\userException ( JS_ASSY_VALUE_ZERO );
	...
	function DeleteInventoryAssemblies(\core\classes\basis $basis) {
		...
		if ($delAssy->unPost ( 'delete' )) { // unpost the prior assembly
			...
			$messageStack->write_debug ();
			gen_redirect ( html_href_link ( FILENAME_DEFAULT, gen_get_all_get_params ( array (
					'action' 
			) ), 'SSL' ) );
			// *************** END TRANSACTION *************************
		}
	...
	function LoadInventoryConnection (\core\classes\basis $basis) {
		\core\classes\messageStack::development ( "executing " . __METHOD__ );
		if (property_exists ( $basis->cInfo, 'sku' ) !== true)
			throw new \core\classes\userException ( "sku variable isn't set can't execute method getPurchasedetails " );
		$sql = $basis->DataBase->query ( "SELECT * FROM " . TABLE_INVENTORY . " i JOIN " . TABLE_INVENTORY_CONNECTIONS . " r ON i.sku = r.sku_1 or i.sku = r.sku_2 WHERE r.sku_1 = '{$basis->cInfo->sku}' OR r.sku_2 = '{$basis->cInfo->sku}'" );
		$sql->execute ();
		$results = $sql->fetchAll ( \PDO::FETCH_ASSOC );
		$basis->cInfo->total = sizeof ( $results );
		$basis->cInfo->rows = $results;
	...
	function DeleteInventoryConnection  (\core\classes\basis $basis) {
		\core\classes\messageStack::development ( "executing " . __METHOD__ );
		if (property_exists ( $basis->cInfo, 'sku_1' ) !== true || property_exists ( $basis->cInfo, 'sku_2' ) !== true)
			throw new \core\classes\userException ( "sku variable isn't set can't execute method getPurchasedetails " );
		$basis->DataBase->exec("DELETE FROM " . TABLE_INVENTORY_CONNECTIONS . " WHERE sku_1 = '{$basis->cInfo->sku_1}' and sku_2 = '{$basis->cInfo->sku_2}' ");
		$basis->cInfo->success = true;
		$basis->cInfo->message = TEXT_DELETED_SUCCESSFULLY;
	}
}
