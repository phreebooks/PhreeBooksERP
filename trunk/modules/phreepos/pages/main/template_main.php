...
if ($security_level > 1) {
  $toolbar->add_icon('pos_return', 'onclick="location.href = \'' . html_href_link(FILENAME_DEFAULT, gen_get_all_get_params(array('action', 'jID')) . '&action=pos_return&jID='.$order->journal_id, 'SSL') . '\'"', 50);
...
<div class="easyui-dialog" data-options="closed: true," id="customer_div" title="<?php echo TEXT_CUSTOMER; ?>" style="height:450px;width:700px">
	...
	<?php
	  ...
	  echo '<li><label>' . html_pull_down_menu('bill_country_code', $_SESSION['language']->get_countries_dropdown(), $order->bill_country_code ? $order->bill_country_code : COMPANY_COUNTRY) . '</label></li>';
