	...
	if (is_array($temp)) {
		...
		foreach ($temp as $key => $value) {
			...
			if (is_array($value)) foreach ($value as $carrier => $prices) {
				...
				echo '<td align="right">' . $admin->currencies->format($prices['quote']) . '</td>' . chr(10);
				echo '<td align="right">' . $admin->currencies->format($prices['book']) . '</td>' . chr(10);
				echo '<td align="right">' . $admin->currencies->format($prices['cost']) . '</td>' . chr(10);
