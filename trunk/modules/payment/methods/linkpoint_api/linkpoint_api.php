...
class linkpoint_api extends \payment\classes\payment {
  ...
  public function __construct(){
  	global $order;
  	parent::__construct();
	if ($this->enabled && !function_exists('curl_init')) throw new \core\classes\userException(MODULE_PAYMENT_LINKPOINT_API_TEXT_ERROR_CURL_NOT_FOUND);
...
//			$myorder["items"][$i]['invoice_num'] = $order->item_rows[$i]['inv'];
		...
		if (trim($result) == '<r_approved>FAILURE</r_approved><r_error>Could not connect.</r_error>' || !is_array($result)) {
			throw new \core\classes\userException(MODULE_PAYMENT_LINKPOINT_API_TEXT_FAILURE_MESSAGE);
...
*/
		...
		if ($result["r_approved"] != "APPROVED") {
			...
			if (substr($result['r_error'], 0, 10) == 'SGS-020005') throw new \core\classes\userException($result['r_error']);
			if (substr($result['r_error'], 0, 10) == 'SGS-005000') throw new \core\classes\userException( MODULE_PAYMENT_LINKPOINT_API_TEXT_GENERAL_ERROR . '<br />' . $result['r_error']); // The server encountered a database error
			if (substr($result['r_error'], 0, 10) == 'SGS-000001' || strstr($result['r_error'], 'D:Declined') || strstr($result['r_error'], 'R:Referral')) throw new \core\classes\userException(MODULE_PAYMENT_CC_TEXT_DECLINED_MESSAGE . '<br />' . $result['r_error']);
			if (substr($result['r_error'], 0, 10) == 'SGS-005005' || strstr($result['r_error'], 'Duplicate transaction')) throw new \core\classes\userException(MODULE_PAYMENT_LINKPOINT_API_TEXT_DUPLICATE_MESSAGE . '<br />' . $result['r_error']);
		}
		...
		if ($result["r_approved"] != "APPROVED") {
			// alert to customer:
			throw new \core\classes\userException(MODULE_PAYMENT_CC_TEXT_DECLINED_MESSAGE);
	...
	function after_order_create($zf_order_id) {
		...
		$admin->DataBase->queryute("update " . TABLE_LINKPOINT_API . " set order_id ='" . $zf_order_id . "' where lp_trans_num = '" . $lp_trans_num . "'");
	}
	...
	function admin_notification($zf_order_id) {
		...
		$lp_api = $admin->DataBase->query($sql);
		if ($lp_api->fetch(\PDO::FETCH_NUM) > 0)
	...
	function check() {
		...
		if (!isset ($this->_check)) {
			$check_query = $admin->DataBase->query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_LINKPOINT_API_STATUS'");
  ...
  function install() {
	...
	if (!$admin->DataBase->table_exists(TABLE_LINKPOINT_API)) {
  	...
  	function delete() {
		...
		if ($admin->DataBase->table_exists(TABLE_LINKPOINT_API)) { // cleanup database if contains no data
		  	...
		  	if ($result->fetch(\PDO::FETCH_NUM) != 0) throw new \core\classes\userException("Can't delete table ". TABLE_LINKPOINT_API. " because it contains data", $code, $previous);
	...
	function _doRefund($oID, $amount = 0) {
		...
		if (isset ($_POST['refconfirm']) && $_POST['refconfirm'] != 'on') throw new \core\classes\userException(MODULE_PAYMENT_LINKPOINT_API_TEXT_REFUND_CONFIRM_ERROR);
		if (isset ($_POST['buttonrefund']) && $_POST['buttonrefund'] == MODULE_PAYMENT_LINKPOINT_API_ENTRY_REFUND_BUTTON_TEXT) {
			...
			$new_order_status = (int) MODULE_PAYMENT_LINKPOINT_API_REFUNDED_ORDER_STATUS_ID;
			if ($refundAmt == 0) throw new \core\classes\userException(MODULE_PAYMENT_LINKPOINT_API_TEXT_INVALID_REFUND_AMOUNT);
		}
		if (isset ($_POST['cc_number']) && (int) trim($_POST['cc_number']) == 0) throw new \core\classes\userException(MODULE_PAYMENT_LINKPOINT_API_TEXT_CC_NUM_REQUIRED_ERROR);
		...
		$query = $admin->DataBase->query($sql);
		if ($query->fetch(\PDO::FETCH_NUM) < 1) throw new \core\classes\userException(MODULE_PAYMENT_LINKPOINT_API_TEXT_NO_MATCHING_ORDER_FOUND);
		/**
		...
		if ($failure) {
			throw new \core\classes\userException($response_alert);
		} else {
	...
	function _doCapt($oID, $amt = 0, $currency = 'USD') {
		...
		if (isset ($_POST['captconfirm']) && $_POST['captconfirm'] == 'on') {
			...
			throw new \core\classes\userException(MODULE_PAYMENT_LINKPOINT_API_TEXT_CAPTURE_CONFIRM_ERROR);
		}
		...
		if (isset ($_POST['btndocapture']) && $_POST['btndocapture'] == MODULE_PAYMENT_LINKPOINT_API_ENTRY_CAPTURE_BUTTON_TEXT) {
			if ($captureAmt == 0) throw new \core\classes\userException(MODULE_PAYMENT_LINKPOINT_API_TEXT_INVALID_CAPTURE_AMOUNT);
		}
		/**
	...
	function _doVoid($oID, $note = '') {
		...
		if (isset ($_POST['ordervoid']) && $_POST['ordervoid'] == MODULE_PAYMENT_LINKPOINT_API_ENTRY_VOID_BUTTON_TEXT) {
			if (isset ($_POST['voidconfirm']) && $_POST['voidconfirm'] != 'on') throw new \core\classes\userException(MODULE_PAYMENT_LINKPOINT_API_TEXT_VOID_CONFIRM_ERROR);
		}
