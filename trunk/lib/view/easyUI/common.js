...
/* use number settings for this, formatCurrency handles the displays
if (typeof bizDefaults.currency !== 'undefined') {
	jq.fn.numberbox.defaults.precision       = bizDefaults.currency.currencies[bizDefaults.currency.defaultCur].dec_len;
	jq.fn.numberbox.defaults.decimalSeparator= bizDefaults.currency.currencies[bizDefaults.currency.defaultCur].dec_pt;
	jq.fn.numberbox.defaults.groupSeparator  = bizDefaults.currency.currencies[bizDefaults.currency.defaultCur].sep;
	if (bizDefaults.currency.currencies.length > 1) {
		jq.fn.numberbox.defaults.prefix      = bizDefaults.currency.currencies[bizDefaults.currency.defaultCur].prefix;
		jq.fn.numberbox.defaults.suffix      = bizDefaults.currency.currencies[bizDefaults.currency.defaultCur].suffix;
	}
}
*/
 ...
 * @param string amount - Locale formatted curency value
 ...
 */
function formatCurrency(amount, pfx_sfx) { // convert to expected currency format
...
//alert('found currency = '+currency+' and decimal point = '+dec_pt+' and separator = '+sep);
	...
	var wholeNum = parseFloat(amount);
