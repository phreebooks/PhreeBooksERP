...
if (!defined('PAYMENT_PAYMENTECH_WSDL'))    { define('PAYMENT_PAYMENTECH_WSDL',    '3.9'); }
...
class paymentech
{
    ...
    public function __construct()
    {
        ...
        $this->settings= ['cash_gl_acct'=>$pmtDef['gl_payment_c'],'disc_gl_acct'=>$pmtDef['gl_discount_c'],'order'=>10,'merchant_bin'=>'','merchant_id'=>'','user_name'=>'',
            'user_pass'=>'','auth_type'=>'Authorize/Capture','prefix'=>'CC','prefixAX'=>'AX','allowRefund'=>'0'];
    ...
    public function settingsStructure()
    {
        ...
        return [
            ...
            'merchant_bin'=> ['label'=>$this->lang['merchant_bin'], 'position'=>'after','attr'=>['type'=>'text', 'size'=>'20','value'=>$this->settings['merchant_bin']]],
            'merchant_id' => ['label'=>$this->lang['merchant_id'],  'position'=>'after','attr'=>['type'=>'text', 'size'=>'20','value'=>$this->settings['merchant_id']]],
            'user_name'   => ['label'=>$this->lang['username'],     'position'=>'after','attr'=>['type'=>'text', 'size'=>'20','value'=>$this->settings['user_name']]],
            'user_pass'   => ['label'=>$this->lang['password'],     'position'=>'after','attr'=>['type'=>'text','value'=>$this->settings['user_pass']]],
...
//                $desc['hint']  = isset($desc['hint']) ? $desc['hint'] : '****';
            ...
            case 'n': // new card
                $refs = $this->guessInv($ledger);
                $submit_data = [
                    'trans_type'                => 'AC', // 'A' for authorize
                    'hostedSecureID'                      => $this->settings['merchant_bin'], // Assigned by Chase Paymentech. 000001 Stratus, 000002 PNS
                    'return_url'               => $this->settings['merchant_id'], // Gateway merchant account number assigned by Chase Paymentech
                    'content_template_url'               => $this->settings['terminal_id'], // Merchant Terminal ID assigned by Chase Paymentech
                    'collectAddress'=> $this->settings['user_name'],
                    'total_amt'=> $this->settings['user_pass'],
                    'order_id'           => PAYMENT_PAYMENTECH_WSDL,
                    'customer_firstname'             => 'EC',
                    'customer_lastname'             => $fields['number'],
                    'customer_city'                    => $fields['month'] . substr($fields['year'], -2), // Format: YYYYMM
                    'customer_postal_code'                   => $ledger->main['total_amount'],
//&hostedSecureID=cpt00000000
//&hostedSecureAPIToken=abcd1234abcd1234abcd1234abcd1234
//&callback_url=https://www.website.com/crehpfcallback.html
//&css_url=
//&action=buildForm
...
//                    'avsPhone'                 => substr(preg_replace("/[^0-9]/", "", $ledger->main['telephone1_b']), 0, 14),
    ...
    private function queryMerchant($request=[])
    {
        ...
        msgDebug("\nSending to url: $url the string: ".print_r($request, true));
        ...
        try {
            $resp = $client->NewOrder($request);
            msgDebug("\nReceived back from Paymentech wsdl: ".print_r($resp, true));
            if (!empty($resp->success)) {
                msgAdd("Response successful, response = ".print_r($resp, true));
            } else { //handle errors
                msgAdd('Paymentech error, response: '.htmlspecialchars($client->__getLastResponse()));
            }
        } catch (SoapFault $exception) {
            msgAdd("[soap fault] ({$exception->faultcode}) {$exception->faultstring}");
        }
return false;
