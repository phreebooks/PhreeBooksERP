 ...
 * @version    2.x Last Update: 2018-05-18
...
//icnAction is needed for datagrids with action icon rows. The reason is when clicking a row, the icon onClick event is 
//handled before the datagrid onClick events and therefore if a row is selected and the user clicks an icon on another row,
//the icon event will act upon the previously selected row. See lib/controller/module/phrebooks/design method dgOrders for example
var icnAction = '';
...
function formValidate() { // check form
	...
	clearInnerLabels(addressFields, '_b');
	clearInnerLabels(addressFields, '_s');
	return true;
}
...
//alert('jID = '+jID+' and rID = '+rID+' and action = '+action+' and xAction = '+xAction);
	if (jq('#winStatus')) { jq('#winStatus').window('close'); }
...
function contactsDetail(rID, suffix, fill) {
    jq.ajax({
        ...
        success: function(json) {
            ...
            jq('#AddUpdate'+suffix).prop('checked', false);
...
function ordersPricing(idx, sku, qty, type) {
	...
	jq.ajax({
		...
		success: function (data) {
			...
			if (!priceEditor) return; // no editor, just return
//alert('returned with price = '+data.price);
...
//alert('setting price = '+data.price+' and total = '+(data.price*qty));
			jq(priceEditor.target).numberbox('setValue', data.price);
			var totalEditor = jq('#dgJournalItem').datagrid('getEditor', {index:idx,field:'total'});
			jq(totalEditor.target).numberbox('setValue', data.price*qty);
...
function ordersEditing(rowIndex) {
	...
	if (!sku && !desc) { // blank row, set the defaults
		...
		var taxEditor = jq('#dgJournalItem').datagrid('getEditor', {index:rowIndex,field:'tax_rate_id'});
		jq(taxEditor.target).combogrid('setValue',def_contact_tax_id);
	}
    ...
    switch (bizDefaults.phreebooks.journalID) { // disable sku editor if linked to SO/PO or at least part of line has been filled
            ...
            case 10:
                ...
                if (typeof bal !== 'undefined' && bal > 0) {
                    jq(skuEditor.target).combogrid({readonly:true}).combogrid('setValue',sku).combogrid('setText',sku);
                }                
                break;
            default:
...
function ordersCalc(action) {
	...
	if (!qtyEditor || !priceEditor || !totalEditor) return; // editor is not active
	var oldQty  = jq('#dgJournalItem').edatagrid('getRows')[curIndex]['qty'];
	var newQty  = cleanNumber(qtyEditor.target.val());
	if (isNaN(newQty))   newQty   = 0;
	var newPrice= cleanCurrency(priceEditor.target.val());
...
//	alert('ordersCalc action = '+action+' and oldQty = '+oldQty+' and newQty = '+newQty+' and newPrice = '+newPrice+' and newTotal = '+newTotal);
	switch (action) {
		case 'qty':
            ...
            // when commented, automatically opens SO/PO when closed, user may not observe that it was re-openedand when saved, SO/PO is re-opened.
//			if (oldQty !== newQty) jq('#closed').attr('checked', false);
            ...
            if (hasSOorPO) { // don't change price as it was set in the SO/PO, just update total
                jq('#dgJournalItem').edatagrid('getRows')[curIndex]['total'] = newPrice*newQty;               
			    jq(totalEditor.target).numberbox('setValue', newPrice*newQty);
...
function selPayment() {
	...
	var value = jq("#method_code>option:selected").val();
...
function assyUpdateBalance() {
	var onHand = parseFloat(jq('#qty_stock').val());
	if (isNaN(onHand)) {
		jq('#qty_stock').val('0');
		onHand = 0;
	}
	var qty = parseFloat(jq('#qty').val());
	if (isNaN(qty)) {
		jq('#qty').val('1');
		qty = 1;
	}
