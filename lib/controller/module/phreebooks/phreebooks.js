...
//          alert('iso = '+iso+' and xRate = '+xRate+' and price = '+data.price);
            pbSetPrice(idx, data.price*xRate);
            pbSetTotal(idx, data.price*qty*xRate);
...
//            if (oldQty !== newQty) jq('#closed').attr('checked', false);
            ...
            } else if (oldQty !== newQty) { // fetch a new price based on the qty change
...
function ordersCurrency(newISO, oldISO) {
    bizTextSet('currency_rate', bizDefaults.currency.currencies[newISO].value);
...
//alert('iso = '+newISO+' dec = '+dec+' sep = '+sep+' len = '+len+' value = '+rate+' prefix = '+pfx+' suffix = '+sfx);
    ...
    for (var rowIndex=0; rowIndex<rowData.total; rowIndex++) {
        ...
        newTotal = rowData.rows[rowIndex].total * rate;
...
//alert('bankingCalc action = '+action+' and newDisc = '+newDisc+' and newTotal = '+newTotal);
    switch (action) {
        case 'disc':
            var amount  = cleanCurrency(jq('#dgJournalItem').edatagrid('getRows')[curIndex]['amount']);
...
function reconInit(row, data) {
    ...
    bizTextSet('stmt_balance', stmtBal);
    pauseTotal = true;
