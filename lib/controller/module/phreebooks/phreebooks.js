...
function setDgEdVal(idx, fld, amount) {
    ...
    if (ed) {
        jq(ed.target).numberbox('setValue', amount);
        jq('#dgJournalItem').edatagrid('getRows')[idx][fld] = amount;
    }
}
...
function setDgEdTxt(idx, fld, txt) {
    ...
    if (ed) {
        ed.target.val(txt);
        jq('#dgJournalItem').edatagrid('getRows')[idx][fld] = txt;
    }
}
...
//          if (oldQty !== newQty) jq('#closed').attr('checked', false);
        ...
        case 'total':
            ...
            var tmp1 = (newPrice, false); // check for rounding circular logic
            var tmp2 = (jq('#dgJournalItem').edatagrid('getRows')[curIndex]['price'], false);
