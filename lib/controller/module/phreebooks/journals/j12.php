...
/*******************************************************************************************************************/
    ...
    public function getDataMain(&$structure)
    {
        if ($this->action == 'inv') { // clear some fields to convert purchase/sales order or quote to receive/invoice
            ...
            if (getModuleCache('extShipping', 'properties', 'status')) { $this->main['waiting'] = '1'; } // set waiting to ship flag
...
//			$this->main['rep_id'] = getUserCache('profile', 'contact_id', false, '0');
    ...
    public function customizeView(&$data, $rID=0, $cID=0, $security=0)
    {
        ...
        $data['divs']['divDetail'] = ['order'=>50,'type'=>'divs','classes'=>['areaView'],'attr'=>['id'=>'pbDetail'],'divs'=>[
            'billAD' => ['order'=>20,'label'=>lang('address_book_type_b'),'type'=>'address','label'=>lang('bill_to'),'classes'=>['blockView'],'attr'=>['id'=>'address_b'],'content'=>$this->cleanAddress($data['fields'], '_b'),
                'settings'=>['suffix'=>'_b','search'=>true,'copy'=>true,'update'=>true,'validate'=>true,'fill'=>'both','required'=>true,'store'=>false]],
            'shipAD' => ['order'=>30,'label'=>lang('address_book_type_s'),'type'=>'address','label'=>lang('ship_to'),'classes'=>['blockView'],'attr'=>['id'=>'address_s'],'content'=>$this->cleanAddress($data['fields'], '_s'),
