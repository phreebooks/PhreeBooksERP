...
class phreeformRender 
{
    ...
    public function open(&$layout=[])
    {
		...
		$data  = ['type'=>'page','title'=> $report->title,
            ...
            'jsReady'=> ['init'=>$this->getViewRenderJS()]];
		if ($rID) {
			$data['report']      = $report;
    ...
    private function getViewRender($viewData)
    {
        ...
        $output .= '   <thead><tr><th colspan="3">'."</th></tr></thead>\n";
        $output .= '   <tbody>'."\n";
        $output .= '    <tr>'."\n";
        $output .= '	  <td>'.html5('fromName',  $viewData['fields']['fromName']) ."</td>\n";
        $output .= '	  <td>'.html5('fromEmail', $viewData['fields']['fromEmail'])."</td>\n";
        $output .= '    </tr>'."\n";
        $output .= '    <tr>'."\n";
        $output .= '	  <td>'.html5('toName',  $viewData['fields']['toName']) ."</td>";
        $output .= '	  <td>'.html5('toEmail', $viewData['fields']['toEmail'])."</td>\n";
        $output .= '    </tr>'."\n";
        $output .= '    <tr>'."\n";
        $output .= '      <td>'.html5('CCName',  $viewData['fields']['CCName']) ."</td>\n";
        $output .= '      <td>'.html5('CCEmail', $viewData['fields']['CCEmail'])."</td>\n";
        $output .= '    </tr>'."\n";
        $output .= '    <tr><td colspan="2">'.html5('msgSubject', $viewData['fields']['msgSubject']).'</td></tr>'."\n";
        ...
        $output .= '    <tr><td colspan="2">'.html5('msgBody', $viewData['fields']['msgBody']).'</td></tr>'."\n";
        $output .= '   </tbody>'."\n";
        $output .= '  </table>'."\n";
        $output .= '</div>'."\n";
        ...
        if (sizeof($viewData['fields']['reports']) > 1) {
          ...
          foreach ($viewData['fields']['reports'] as $value) {
            $output .= "	<div>".html5('rID',['events'=>['onChange'=>"phreeformBody();"],'attr'=>['type'=>'radio','value'=>$value['id']]]).'&nbsp;'.$value['text']."</div>\n";
          }
          $output .= "	</div>\n";
    ...
    {
        return "jq('#frmPhreeform').submit(function (e) {
