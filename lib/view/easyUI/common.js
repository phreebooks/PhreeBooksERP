...
jq.extend(jq.fn.textbox.methods, {
	show: function(jqy) { return jqy.each(function() { jq(this).next().show(); }); },
	hide: function(jqy) { return jqy.each(function() { jq(this).next().hide(); }); }
});
 ...
 */
jq.extend(jq.fn.datagrid.defaults, {
	autoUpdateDetail: true  // Define if update the row detail content when update a row
});

var detailview = jq.extend({}, jq.fn.datagrid.defaults.view, {
	type: 'detailview',
	render: function(target, container, frozen){
		var state = jq.data(target, 'datagrid');
		var opts = state.options;
		if (frozen){
		...
		var fields = jq(target).datagrid('getColumnFields', frozen);
		var table = [];
		...
		jq(container).html(table.join(''));
	},
	
	renderRow: function(target, fields, frozen, rowIndex, rowData){
		var opts = jq.data(target, 'datagrid').options;
		...
		for(var i=0; i<fields.length; i++){
			var field = fields[i];
			var col = jq(target).datagrid('getColumnOption', field);
			if (col){
	...
	insertRow: function(target, index, row){
		var opts = jq.data(target, 'datagrid').options;
		var dc = jq.data(target, 'datagrid').dc;
		var panel = jq(target).datagrid('getPanel');
		var view1 = dc.view1;
		var view2 = dc.view2;
		
		var isAppend = false;
		var rowLength = jq(target).datagrid('getRows').length;
		if (rowLength == 0){
			jq(target).datagrid('loadData',{total:1,rows:[row]});
			return;
		}
		...
		jq.fn.datagrid.defaults.view.insertRow.call(this, target, index, row);
	...
	deleteRow: function(target, index){
		...
		tr.next().remove();
		jq.fn.datagrid.defaults.view.deleteRow.call(this, target, index);
	...
	updateRow: function(target, rowIndex, row){
		var dc = jq.data(target, 'datagrid').dc;
		var opts = jq.data(target, 'datagrid').options;
		var cls = jq(target).datagrid('getExpander', rowIndex).attr('class');
		jq.fn.datagrid.defaults.view.updateRow.call(this, target, rowIndex, row);
		jq(target).datagrid('getExpander', rowIndex).attr('class',cls);
		
		// update the detail content
		if (opts.autoUpdateDetail && this.canUpdateDetail){
			var row = jq(target).datagrid('getRows')[rowIndex];
			var detail = jq(target).datagrid('getRowDetail', rowIndex);
	...
	bindEvents: function(target){
		...
		var clickHandler = (jq.data(body[0],'events')||jq._data(body[0],'events')).click[0].handler;
		body.unbind('click.detailview').bind('click.detailview', function(e){
			...
			if (tt.hasClass('datagrid-row-expander')){
				...
				if (tt.hasClass('datagrid-row-expand')){
					jq(target).datagrid('expandRow', rowIndex);
				} else {
					jq(target).datagrid('collapseRow', rowIndex);
				}
				jq(target).datagrid('fixRowHeight');
				e.stopPropagation();
	...
	onBeforeRender: function(target){
		...
		if (!hasExpander){
			...
			var td = jq('<td rowspan="'+opts.frozenColumns.length+'"><div class="datagrid-header-expander" style="width:24px;"></div></td>');
			if (jq('tr',t).length == 0){
				td.wrap('<tr></tr>').parent().appendTo(jq('tbody',t));
	...
	onAfterRender: function(target){
		...
		jq.fn.datagrid.defaults.view.onAfterRender.call(this, target);
		...
		if (!state.onResizeColumn){
			...
			opts.onResizeColumn = function(field, width){
				...
				var rowCount = jq(target).datagrid('getRows').length;
				for(var i=0; i<rowCount; i++){
					jq(target).datagrid('fixDetailRowHeight', i);
				}
...
jq.extend(jq.fn.datagrid.methods, {
	fixDetailRowHeight: function(jqy, index){
		return jqy.each(function(){
			...
			var dc = jq.data(this, 'datagrid').dc;
	...
	getExpander: function(jqy, index){	// get row expander object
		var opts = jq.data(jqy[0], 'datagrid').options;
	...
	expandRow: function(jqy, index){
		return jqy.each(function(){
			var opts = jq(this).datagrid('options');
			var dc = jq.data(this, 'datagrid').dc;
			var expander = jq(this).datagrid('getExpander', index);
			if (expander.hasClass('datagrid-row-expand')){
				...
				tr1.show();
				tr2.show();
				jq(this).datagrid('fixDetailRowHeight', index);
				if (opts.onExpandRow){
					var row = jq(this).datagrid('getRows')[index];
	...
	subgrid: function(jqy, conf){
		return jqy.each(function(){
			...
			function createGrid(target, conf, prow){
				...
				jq(target)[plugin](jq.extend({}, conf.options, {
					...
					onExpandRow: function(index, row){
						var opts = jq(this).datagrid('options');
						var rd = jq(this).datagrid('getRowDetail', index);
					...
					onResize: function(){
						var dg = jq(this).children('div.datagrid-view').children('table')
			...
			function getSubGrid(rowDetail){
				var div = jq(rowDetail).children('div');
			...
			function setParentHeight(target){
				var tr = jq(target).closest('div.datagrid-row-detail').closest('tr').prev();
				if (tr.length){
			...
			function setHeight(target, index){
				jq(target).datagrid('fixDetailRowHeight', index);
				jq(target).datagrid('fixRowHeight', index);
				var tr = jq(target).closest('div.datagrid-row-detail').closest('tr').prev();
				if (tr.length){
					var index = parseInt(tr.attr('datagrid-row-index'));
					var dg = tr.closest('div.datagrid-view').children('table');
...
//alert('accID = '+accID+' and dgID = '+dgID+' and divID = '+divID+' and title = '+title+' and path = '+path+' and rID = '+rID+' and action = '+action);
    ...
    jq('#'+divID).panel({href:bizunoAjax+'&bizRt='+xVars});
    ...
    switch (json.action) {
        ...
        case 'window':
            ...
            if (isMobile()) {
                ...
                if (typeof json.href !== 'undefined') { jq('#navPopupBody').load(json.href, function() { jq.parser.parse('#navPopup'); } ); }
            } else {
                var id       = typeof json.id   != 'undefined' ? json.id : 'win'+Math.floor((Math.random() * 1000000) + 1);
                if (jq("#"+id).hasClass( "easyui-window" )) { jq('#'+id).window('destroy', true); }
                ...
                jq('#'+id).window({ title:title, top:winT, left:winL, width:winW, height:winH, modal:true }); //.window('center');
                if (typeof json.html == 'undefined') { json.html = ''; }
                jq('#'+id).html(json.html);
                jq.parser.parse('#'+id);
                if (typeof json.href != 'undefined') { jq('#'+id).window( {href:json.href } ); }
                if (wClosable) { jq( "div.panel-tool" ).css("display", "inline-block"); } // mobile.css hides this, so show it
            }
            break;
...
function dashDelay(dashPath, dashID, el) {
    jq('#'+el).on('keyup', {dashPath: dashPath, dashID: dashID}, function(event) {
        clearTimeout(dashTimer);
        dashTimer = setTimeout(function () { dashSubmit(event.data.dashPath, event.data.dashID); }, dashTimerVal);
    });
    jq('#'+el).on('keydown', function () { clearTimeout(dashTimer); });
}
...
/**
 * DEPRECATE THIS AFTER 2020-09-01
 */
function dashboardAttr(dashPath, dashID) { dashSubmit(dashPath, dashID); }
...
function cronInit(baseID, urlID) {
    ...
    jq.ajax({ url:bizunoAjax+'&bizRt='+urlID, async:false, success:cronResponse });
}
...
function cronResponse(json) {
    ...
    if (json.percent < 100) {
        ...
        jq( "div.panel-tool" ).css("display", "inline-block");
    }
}
...
function imgManagerInit(imgID, src, srcPath, myFolder)
{
    ...
    divInvImg      += '  <img type="img" style="max-height:100%;max-width:100%;" src="'+bizunoAjaxFS+'&src=0/'+path+'" name="img_'+imgID+'" id="img_'+imgID+'" /></a></div><div id="imdtl_'+imgID+'"></div>';
...
function bizCheckBoxGet(id) {
    if (jq("#"+id).hasClass( "easyui-switchbutton" )) {
        return jq('#'+id).switchbutton('options').checked == true ? 1 : 0;
...
function bizDGgetRow(dgID, idx) {
    var row = jq('#'+dgID).datagrid('getRows')[idx];
    return row;
}
...
function bizFocus(id, dgID) {
    if (jq('#'+id).hasClass("easyui-textbox") || jq('#'+id).hasClass("easyui-passwordbox")) {
        jq('#'+id).textbox('textbox').focus();
    } else if (jq('#'+id).hasClass("easyui-combobox")) {
        jq('#'+id).combobox('textbox').focus();
    } else if (jq("#"+id).hasClass("easyui-combogrid")) {
        jq('#'+id).combogrid('textbox').focus();
    } else {
        jq('#'+id).focus();
    }
    if (typeof dgID == 'string') {
        jq('#'+dgID+'Toolbar').keypress(function (e) { if (e.keyCode == 13) { window[dgID+'Reload'](); } });
    }
}
...
function bizGridExists(id) {
    var dg = jq('#'+id);
    if (dg.data('datagrid')) { return true; }
    return false;
}
 ...
 * @returns null
 */
function bizGridReload(id) { jq('#'+id).datagrid('reload'); }
...
function bizGridSerializer(dGrid, fld) {
    if (jq('#'+dGrid).length) {
        jq('#'+dGrid).edatagrid('saveRow');
        var items = jq('#'+dGrid).datagrid('getData');
        var serializedItems = JSON.stringify(items);
        jq('#'+fld).val(serializedItems);
    }
}
...
function bizNumEdSet(id, idx, fld, amount) {
    ...
    var ed = jq('#'+id).edatagrid('getEditor', {index:idx,field:fld});
    jq('#'+id).edatagrid('getRows')[idx][fld] = amount; // needs to set irregardless of editor (i.e. when editors are hidden)
...
function bizParse(id) {
    if (typeof id=='undefined') { jq.parser.parse(); }
    else                        { jq.parser.parse('#'+id); }
}
...
function bizSelVals(id, values) {
    if (!jq('#'+id).combobox({})) { return; }
    jq('#'+id).combobox('loadData', values);
}
 ...
 * @returns null
 */
function bizTreeReload(id) { jq('#'+id).tree('reload'); }
...
function addressClear(suffix) {
    ...
    jq.each(contactFields, function (index, value) { bizTextSet(value+suffix, ''); });
...
function shippingValidate(suffix) {
    ...
    jsonAction('extShipping/address/validateAddress&suffix='+suffix+'&methodCode='+code+'&country='+country, 0, ship);
}
...
function journalEditSel(jID, rID) {
    switch (jID) {
        ...
        case 10:
        case 12:
        case 13: jsonAction('phreebooks/main/getJournalSel&jID='+jID, rID); break;
...
//alert('jID = '+jID+' and rID = '+rID+'and cID = '+cID+' and action = '+action+' and xAction = '+xAction);
    ...
    if (p) {
        ...
        jq('#divJournalDetail').panel({href:bizunoAjax+'&bizRt=phreebooks/main/edit'+xVars});
...
function pbSetPrompt(id, idMin, idMax) {
    ...
    switch (val) {
        case 'all':  jq('#'+idMin).textbox('hide');  jq('#'+idMax).textbox('hide'); break;
        case 'band': jq('#'+idMin).textbox({prompt:jq.i18n('FROM')}).textbox('show');  jq('#'+idMax).textbox({prompt:jq.i18n('TO')}).textbox('show'); break;
        case 'eq':   jq('#'+idMin).textbox({prompt:jq.i18n('VALUE')}).textbox('show'); jq('#'+idMax).textbox('hide'); break;
        case 'not':  jq('#'+idMin).textbox({prompt:jq.i18n('VALUE')}).textbox('show'); jq('#'+idMax).textbox('hide'); break;
        case 'inc':  jq('#'+idMin).textbox({prompt:jq.i18n('VALUE')}).textbox('show'); jq('#'+idMax).textbox('hide'); break;
    }
}
...
//  alert('ordersPricing idx = '+idx+' and sku = '+sku+' and qty = '+qty+' and type = '+type);
    jq.ajax({
        url: bizunoAjax+'&bizRt=inventory/prices/quote&type='+type+'&cID='+cID+'&sku='+sku+'&qty='+qty,
...
function ordersCalc(action) {
    ...
    var oldPrice = jq('#dgJournalItem').edatagrid('getRows')[curIndex]['price'];
    var oldTotal = jq('#dgJournalItem').edatagrid('getRows')[curIndex]['total'];
    switch (action) {
        case 'qty':
...
function inventoryProperties(rowData) {
    if (typeof rowData.sku == 'undefined') { return; }
    windowEdit('inventory/main/properties&data='+rowData.sku, 'winInvProps', jq.i18n('PROPERTIES'), 800, 600);
}
...
function shippingEstimate() {
    ...
    var href = bizunoAjax+'&bizRt=extShipping/ship/rateMain&resi='+resi+'&data='+encodeURIComponent(JSON.stringify(data));
    var json = { action:'window', id:'shippingEst', title:jq.i18n('SHIPPING_ESTIMATOR'), width:1000, height:600, href:href };
    processJson(json);
}

function selPayment(value) {
    if (value == '') return;
    for (index = 0; index < sel_method_code.length; index++) { jq("#div_"+sel_method_code[index].id).hide('slow'); }
...
function adjFill(data) {
    ...
    jq('#dgJournalItem').edatagrid('getRows')[curIndex]['unit_cost'] = parseFloat(data.item_cost); // since hidden
