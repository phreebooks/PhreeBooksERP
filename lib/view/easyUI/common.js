 ...
 * @version    3.x Last Update: 2018-08-15
...
function ajaxForm(formID, skipPre) {
    jq('#'+formID).submit(function (e) {
        ...
        // @todo patch for Safair 11.x browsers hanging on forms submits with EMPTY file fields.
        // can be removed AFTER Apple fixes the problem
        if (navigator.userAgent.indexOf('Safari') !== -1) {
            jq('#'+formID).find("input[type=file]").each(function(index, field) {
                if (jq('#'+field.id).val() == '') { frmData.delete(field.id); }
            });
        }
        jq.ajax({
