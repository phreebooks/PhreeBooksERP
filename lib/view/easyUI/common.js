 ...
 * @version    3.x Last Update: 2019-03-03
...
function ajaxForm(formID, skipPre) {
    jq('#'+formID).submit(function (e) {
        ...
        // @todo patch for Safari 11+  browsers hanging on forms submits with EMPTY file fields.
        // can be removed AFTER Apple fixes the problem
//      if (navigator.userAgent.indexOf('Safari') !== -1) {
//        jq('#'+formID).find("input[type=file]").each(function(index, field) { if (jq('#'+field.id).val() == '') { frmData.delete(field.id); } });
//      }
        jq.ajax({
...
function formatCurrency(amount, pfx_sfx, isoCur, excRate) { // convert to expected currency format
    ...
    var pfxneg  = bizDefaults.currency.currencies[currency].pfxneg;
    var sfxneg  = bizDefaults.currency.currencies[currency].sfxneg;
