...
//	dataType: (jq.browser.msie) ? "text" : "json", // not needed for jquery 2.x
	...
	error: function(XMLHttpRequest, textStatus, errorThrown) {
            ...
            if (errorThrown) {
                ...
                var errMessage = XMLHttpRequest.responseText.substring(0, 500); // truncate the message
...
function ajaxForm(formID) {
    jq('#'+formID).submit(function (e) {
        ...
        if (navigator.userAgent.indexOf('11.1 Safari') !== -1 
                || navigator.userAgent.indexOf('11.1.1 Safari') !== -1 
                || navigator.userAgent.indexOf('11.1.2 Safari') !== -1) {
//          jq.messager.alert('Capture', 'Timestamp = '+new Date().getTime());
            jq('#'+formID).find("input[type=file]").each(function(index, field) {
                if (jq('#'+field.id).val() != '') {
                    alert('This version of Safari cannot upload files, you will need to use a different browser or older version of Safari to upload your files. The rest of the fields will not be affected. PhreeSoft is working with Apple to resolve this issue.');
                }
    ...
    jq.each(arrFields, function (index, value) {
        ...
        if (objField) {
            ...
            objField.blur(function() {
                if (objField.val() == '' || objField.val() == labelText) {
...
function addressCopy(fromSuffix, toSuffix) {
    jq.each(addressFields, function (index, value) {
//        alert('field: '+value+' from value: '+jq('#'+value+fromSuffix).val());
        jq('#'+value+toSuffix).val(jq('#'+value+fromSuffix).val()).css({color:'#000000'});
        jq('#'+value+toSuffix).blur();
    });
