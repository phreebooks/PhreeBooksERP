...
<table style="border-collapse:collapse;">
    ...
    <tbody>
        ...
        <tr><td style="width:125px;">'.pullTableLabel(BIZUNO_DB_PREFIX."inventory", 'price').'</td>
            <td style="width:125px;">'.viewFormat($data['values']['price'], 'currency').'</td></tr>
        <tr><td style="width:125px;">'.pullTableLabel(BIZUNO_DB_PREFIX."inventory", 'full_price').'</td>
            <td style="width:125px;">'.viewFormat($data['values']['full'], 'currency') .'</td></tr>';
if (validateSecurity('phreebooks', 'j6_mgr', 1, false)) {
    $output['body'] .= '        <tr><td style="width:125px;">'.pullTableLabel(BIZUNO_DB_PREFIX."inventory", 'item_cost') .'</td>
            <td style="width:125px;">'.viewFormat($data['values']['cost'], 'currency') .'</td></tr>';
}
$output['body'] .= '    </tbody>
</table>'."\n";
if (isset($data['values']['sheets']) && is_array($data['values']['sheets']) && sizeof($data['values']['sheets'])) {
	...
	$output['body'] .= " <tbody>\n";
	foreach ($data['values']['sheets'] as $level) {
